        ...
        try {
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'product_select') {
                ...
                if (product.price === "Custom") {
                    ...
                    if (isVIP) {
                        if (!memoryStats.subscriptions) memoryStats.subscriptions = {}; if (memoryStats.subscriptions[interaction.user.id]) memoryStats.subscriptions[interaction.user.id].expiresAt += 30*24*60*60*1000; else memoryStats.subscriptions[interaction.user.id] = { username: interaction.user.username, expiresAt: Date.now() + 30*24*60*60*1000, notified: false };
