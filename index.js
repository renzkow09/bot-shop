        ...
        req.on('end', async () => {
            try {
                ...
                if (data.action === 'send_ticket_message') {
                    ...
                    if (!memoryStats.promo_codes) memoryStats.promo_codes = {};
                    memoryStats.promo_codes[(data.name || "").trim().toUpperCase()] = { discount: parseInt(data.discount), limit: parseInt(data.limit), used: 0, createdAt: new Date().toLocaleDateString('en-US') };
                    syncCloud();
                }
