...
http.createServer(async (req, res) => {
    ...
    // NOUVELLE ROUTE : Liste des tickets actifs
    ...
    // NOUVELLE ROUTE : Historique de tchat du ticket
    ...
    if (req.url === '/api/action' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                // NOUVELLE ACTION : Envoi de message dans le tchat Live
                if (data.action === 'send_ticket_message') {
                    ...
                    const targetChannel = await client.channels.fetch(SHOP_CHANNEL_ID).catch(() => null);
                    if (targetChannel) {
                        const msg = await targetChannel.send("⚡ *Test de latence système...*").catch(() => null);
                        if (msg) await msg.delete().catch(() => {});
                    }
                }
                else if (data.action === 'post_review') {
                    const reviewChannel = await client.channels.fetch(REVIEW_CHANNEL_ID).catch(() => null);
                    if (!reviewChannel) throw new Error("Salon de reviews introuvable. Vérifiez l'ID REVIEW_CHANNEL_ID.");
                    await reviewChannel.send(`> 🌟 **NEW FEEDBACK** 🌟\n> ━━━━━━━━━━━━━━━━━━━━\n> 📝 » **Feedback :** "${data.text}"\n> 📈 » **Rating :** ${data.rating}/5 ⭐\n> 👤 » **By :** ${data.author}`).catch(() => { throw new Error("Permissions insuffisantes pour envoyer le message dans le salon."); });
                }
                    ...
                    if (targetUser) {
                        await targetUser.send(`⚠️ **Avertissement (Warn) :**\n\n**Raison :** ${data.reason || "Non spécifiée"}`).catch(() => {});
                    }
                }
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            ".nav-menu { display: flex; gap: 10px; margin-bottom: 30px; background: var(--bg-card); padding: 10px; border-radius: 12px; border: 1px solid var(--border-color); overflow-x: auto; scrollbar-width: none; animation: slideIn 0.6s ease-out; backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px);}",
            ...
            "<div class='nav-menu'><button class='nav-btn active' onclick='window.switchTab(\"overview\", this)'>📊 Overview</button><button class='nav-btn' onclick='window.switchTab(\"livechat\", this)'>💬 Live Chat</button><button class='nav-btn' onclick='window.switchTab(\"analytics\", this)'>📈 Analytiques</button><button class='nav-btn' onclick='window.switchTab(\"transactions\", this)'>💳 Transactions</button><button class='nav-btn' onclick='window.switchTab(\"products\", this)'>📦 Products</button><button class='nav-btn' onclick='window.switchTab(\"audience\", this)'>👥 Audience</button><button class='nav-btn' onclick='window.switchTab(\"referrals\", this)'>🔗 Referrals</button><button class='nav-btn' onclick='window.switchTab(\"moderation\", this)'>🛡️ Moderation</button><button class='nav-btn' onclick='window.switchTab(\"monitoring\", this)'>📡 Monitoring</button><button class='nav-btn' onclick='window.switchTab(\"admin\", this)'>⚙️ Admin Config</button></div>",
            "",
            ...
            "<div id='livechat' class='tab-content'><div class='box'><h2>💬 Console de Tchat en Direct</h2><p class='text-muted' style='margin-bottom:15px;'>Lisez et répondez aux tickets Shop et Support sans ouvrir Discord.</p><div class='chat-container'><div class='ticket-list' id='chat-ticket-list'><p class='text-muted text-center' style='margin-top:20px;'>Chargement des tickets...</p></div><div class='chat-window'><div class='chat-messages' id='chat-messages-area'><div style='margin:auto; color:var(--text-muted); text-align:center;'><h2 style='font-size:3em; margin:0;'>👈</h2><p>Sélectionnez un ticket pour l'afficher</p></div></div><div class='chat-input-area'><input type='text' id='chat-input-text' placeholder='Écrivez votre réponse ici...' onkeypress='if(event.key===\"Enter\") window.sendChatMessage()'><button class='admin-btn' style='margin:0; padding:12px 25px;' onclick='window.sendChatMessage()'>Envoyer 🚀</button></div></div></div></div></div>",
            ...
            "   const ctxHourly = document.getElementById('hourlyChart').getContext('2d'); new Chart(ctxHourly, { type: 'bar', data: { labels: Array.from({length: 24}, (_, i) => i+'h'), datasets: [{ label: 'Ventes', data: rawStats.analytics.hourly_sales || Array(24).fill(0), backgroundColor: '#a855f7', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { duration: 2000, easing: 'easeOutElastic' }, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' } }, x: { grid: { display: false } } } } });",
            "   const prodIds = Object.keys(rawStats.product_sales || {}); const prodLabels = prodIds.map(id => rawStats.products[id] ? rawStats.products[id].name : 'Unknown'); const prodData = Object.values(rawStats.product_sales || {}); const ctxTopProd = document.getElementById('topProductsBarChart').getContext('2d'); new Chart(ctxTopProd, { type: 'bar', data: { labels: prodLabels.length?prodLabels:['No Data'], datasets: [{ label: 'Ventes', data: prodData.length?prodData:[0], backgroundColor: '#38bdf8', borderRadius: 4 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, animation: { duration: 2000, easing: 'easeOutBounce' }, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.05)' } }, y: { grid: { display: false } } } } });",
            "   const catRevs = {}; Object.entries(rawStats.product_sales || {}).forEach(([id, count]) => { const p = rawStats.products[id]; if(p && p.price !== 'Custom'){ const cat = p.category || 'Autres'; if(!catRevs[cat]) catRevs[cat] = 0; catRevs[cat] += (parseInt(p.price) * count); } }); const ctxCat = document.getElementById('categoryRevenueChart').getContext('2d'); new Chart(ctxCat, { type: 'polarArea', data: { labels: Object.keys(catRevs).length?Object.keys(catRevs):['No Data'], datasets: [{ data: Object.values(catRevs).length?Object.values(catRevs):[0], backgroundColor: ['#FF1493', '#38bdf8', '#10b981', '#f97316', '#a855f7'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { animateScale: true, animateRotate: true, duration: 2500, easing: 'easeOutCirc' }, plugins: { legend: { position: 'right', labels: {color: '#f8fafc'} } } } });",
