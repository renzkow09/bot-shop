...
http.createServer(async (req, res) => {
    ...
    if ((req.url === '/dashboard' || req.url === '/') && !isAuthenticated) {
        ...
        return res.end("<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no'><title>Nexus Security</title><style>body{font-family:'Inter',sans-serif;background:#062016;color:#ecfdf5;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;}.login-box{background:rgba(6,78,59,0.4);backdrop-filter:blur(24px);padding:50px;border-radius:24px;border:1px solid rgba(16,185,129,0.2);text-align:center;box-shadow:0 20px 50px rgba(0,0,0,0.8), inset 0 0 20px rgba(16,185,129,0.05);width:90%;max-width:420px;box-sizing:border-box;}h2{font-weight:800;letter-spacing:2px;background:linear-gradient(135deg,#fff 0%,#10b981 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}input{background:rgba(0,0,0,0.6);border:1px solid rgba(16,185,129,0.2);color:white;padding:18px;border-radius:12px;font-size:20px!important;text-align:center;letter-spacing:15px;width:100%;max-width:240px;margin:30px auto;outline:none;transition:all 0.3s;display:block;}input:focus{border-color:#10b981;box-shadow:0 0 25px rgba(16,185,129,0.3);transform:scale(1.05);}button{background:linear-gradient(135deg,#10b981 0%,#059669 100%);color:white;border:none;padding:15px 40px;font-size:1.1em;border-radius:12px;cursor:pointer;font-weight:800;width:100%;transition:all 0.3s;text-transform:uppercase;letter-spacing:2px;box-shadow:0 10px 30px rgba(16,185,129,0.3);}button:hover{transform:translateY(-3px);box-shadow:0 15px 40px rgba(16,185,129,0.5);}</style></head><body><div class='login-box'><h2>NEXUS CORE</h2><input type='password' id='pin' maxlength='4' placeholder='••••'><button onclick='login()'>Authenticate</button><p id='err' style='color:#ef4444;display:none;margin-top:20px;font-weight:bold;letter-spacing:1px;'>Access Denied</p></div><script>async function login(){const res=await fetch('/api/login',{method:'POST',body:JSON.stringify({pin:document.getElementById('pin').value})});if(res.ok)location.reload();else document.getElementById('err').style.display='block';} document.getElementById('pin').addEventListener('keypress', e=>{if(e.key==='Enter')login();});</script></body></html>");
    }
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        body { font-family: 'Inter', sans-serif; background-color: var(--bg-main); background-image: radial-gradient(circle at 10% 50%, rgba(16, 185, 129, 0.05), transparent 30%), radial-gradient(circle at 90% 20%, rgba(52, 211, 153, 0.05), transparent 30%); color: var(--text-main); margin: 0; min-height: 100vh; overflow-x: hidden; }",
            "        ::-webkit-scrollbar { width: 8px; height: 8px; } ::-webkit-scrollbar-thumb { background: rgba(16, 185, 129, 0.2); border-radius: 4px; } ::-webkit-scrollbar-thumb:hover { background: var(--accent-green); }",
            ...
            "        @keyframes pulseGlow { 0% { box-shadow: 0 0 10px rgba(16, 185, 129, 0.2); } 100% { box-shadow: 0 0 20px rgba(16, 185, 129, 0.6); } }",
            ...
            "        .btn-icon { background: rgba(2, 44, 34, 0.6); border: 1px solid rgba(255,255,255,0.05); color: #fff; padding: 10px 18px; border-radius: 12px; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); backdrop-filter: blur(20px); font-size: 1rem; }",
            "        .btn-icon:hover { background: rgba(16, 185, 129, 0.1); border-color: var(--accent-green); transform: translateY(-3px); box-shadow: 0 10px 20px rgba(16, 185, 129, 0.15); }",
            ...
            "        .card { background: linear-gradient(180deg, rgba(6, 78, 59, 0.6) 0%, rgba(2, 44, 34, 0.6) 100%); padding: 25px; border-radius: 20px; border: 1px solid rgba(52, 211, 153, 0.1); transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; backdrop-filter: blur(20px); box-shadow: 0 10px 30px rgba(0,0,0,0.3); }",
            "        .card:hover { transform: translateY(-5px); border-color: rgba(52, 211, 153, 0.3); box-shadow: 0 15px 40px rgba(16, 185, 129, 0.2); }",
            "        .card::before { content: ''; position: absolute; top: 0; left: 0; width: 4px; height: 100%; background: var(--accent-green); transition: width 0.3s ease, box-shadow 0.3s ease; }",
            ...
            "        .card .value { font-size: 2em; font-weight: 800; letter-spacing: -1px; text-shadow: 0 0 20px rgba(52, 211, 153, 0.2); }",
            "        .box { background: linear-gradient(180deg, rgba(6, 78, 59, 0.3) 0%, rgba(2, 44, 34, 0.3) 100%); padding: 25px; border-radius: 20px; border: 1px solid rgba(52, 211, 153, 0.1); margin-bottom: 25px; transition: all 0.3s ease; backdrop-filter: blur(20px); box-shadow: 0 10px 30px rgba(0,0,0,0.3); }",
            "        .box h2 { font-size: 1.2em; font-weight: 600; margin-top: 0; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; color: #fff; letter-spacing: 0.5px; border-bottom: 1px solid rgba(52, 211, 153, 0.1); padding-bottom: 15px; }",
            "        table { width: 100%; border-collapse: separate; border-spacing: 0; } th { padding: 15px; text-align: left; color: var(--text-muted); font-size: 0.85em; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid rgba(52, 211, 153, 0.1); } td { padding: 15px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.02); vertical-align: middle; } tr { transition: all 0.2s ease; } tr:hover { background: rgba(52, 211, 153, 0.05); transform: scale(1.01); }",
            "        input, textarea, select { width: 100%; background: rgba(2, 44, 34, 0.6); border: 1px solid rgba(52, 211, 153, 0.2); color: white; padding: 15px; border-radius: 12px; transition: all 0.3s ease; font-family: 'Inter', sans-serif; font-size: 0.95em; }",
            "        input:focus, textarea:focus, select:focus { border-color: var(--accent-green); box-shadow: 0 0 20px rgba(52, 211, 153, 0.2); outline: none; background: rgba(6, 78, 59, 0.6); }",
            "        .admin-btn { background: linear-gradient(135deg, rgba(16, 185, 129, 0.2) 0%, rgba(16, 185, 129, 0.05) 100%); color: white; border: 1px solid rgba(16, 185, 129, 0.3); padding: 12px 24px; border-radius: 12px; cursor: pointer; font-weight: 600; margin-top: 10px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); text-transform: uppercase; letter-spacing: 1px; font-size: 0.85em; backdrop-filter: blur(10px); }",
            "        .admin-btn:hover { border-color: var(--accent-green); box-shadow: 0 5px 20px rgba(16, 185, 129, 0.3); transform: translateY(-2px); background: linear-gradient(135deg, rgba(16, 185, 129, 0.3) 0%, transparent 100%); color: var(--accent-green); }",
            "        .text-green { color: var(--accent-green); text-shadow: 0 0 10px rgba(16, 185, 129, 0.3); } .text-muted { color: var(--text-muted); } .text-blue { color: var(--accent-blue); text-shadow: 0 0 10px rgba(16, 185, 129, 0.3); }",
            "        ",
            "        /* 📦 HOLOGRAPHIC PRODUCT GRID */",
            "        .product-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 25px; }",
            "        .product-card { background: rgba(6, 78, 59, 0.4); border: 1px solid rgba(52, 211, 153, 0.1); border-radius: 20px; padding: 25px; position: relative; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); display: flex; flex-direction: column; justify-content: space-between; backdrop-filter: blur(20px); overflow: hidden; }",
            "        .product-card::before { content:''; position:absolute; top:-50%; left:-50%; width:200%; height:200%; background: radial-gradient(circle, rgba(16, 185, 129, 0.15) 0%, transparent 70%); opacity:0; transition: opacity 0.5s; pointer-events:none; }",
            "        .product-card:hover { transform: translateY(-10px); border-color: var(--accent-green); box-shadow: 0 15px 40px rgba(16, 185, 129, 0.2); }",
            "        .product-card:hover::before { opacity:1; }",
            "        .prod-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px; border-bottom: 1px solid rgba(52, 211, 153, 0.1); padding-bottom: 15px; position: relative; z-index: 2; }",
            ...
            "        .prod-id { font-size: 0.7em; color: var(--accent-green); font-weight: 800; background: rgba(16, 185, 129, 0.1); padding: 4px 10px; border-radius: 12px; letter-spacing: 1px; border: 1px solid rgba(16, 185, 129, 0.2); }",
            ...
            "        .prod-desc { font-size: 0.9em; color: #a7f3d0; line-height: 1.6; margin-bottom: 20px; flex-grow: 1; position: relative; z-index: 2; }",
            "        .prod-link { background: rgba(2, 44, 34, 0.6); padding: 12px; border-radius: 10px; font-size: 0.8em; margin-bottom: 20px; word-break: break-all; border: 1px dashed rgba(52, 211, 153, 0.2); position: relative; z-index: 2; }",
            ...
            "        .feed-container::-webkit-scrollbar { width: 4px; } .feed-container::-webkit-scrollbar-thumb { background: rgba(16, 185, 129, 0.2); border-radius: 4px; }",
            "        .feed-item { display: flex; align-items: flex-start; gap: 15px; margin-bottom: 15px; padding: 15px; background: rgba(52, 211, 153, 0.05); border-left: 3px solid var(--accent-green); border-radius: 0 12px 12px 0; font-size: 0.9em; transition: 0.3s; }",
            "        .feed-item:hover { background: rgba(52, 211, 153, 0.1); transform: translateX(5px); }",
            ...
            "        .feed-time { font-size: 0.75em; color: var(--accent-green); min-width: 60px; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase; }",
            "        ",
            "        /* SPLASH SCREEN FAST (CSS ONLY, NO JS REQUIRED TO HIDE) */",
            "        .splash-screen { position: fixed; inset: 0; background: #021c13; z-index: 9999; display: flex; flex-direction: column; justify-content: center; align-items: center; pointer-events: all; animation: fadeOutSplash 0.5s ease forwards 1.5s; }",
            ...
            "        .loader-bar-fast { width: 250px; height: 3px; background: rgba(16, 185, 129, 0.2); border-radius: 2px; overflow: hidden; position: relative; margin-top: 20px; }",
            "        .loader-bar-fast::after { content: ''; position: absolute; left: 0; top: 0; height: 100%; width: 100%; background: var(--accent-green); animation: loadBarAnim 1.5s ease-in-out forwards; }",
            ...
            "        .kanban-col { background: rgba(2, 44, 34, 0.4); border-radius: 16px; padding: 15px; min-width: 300px; flex: 1; border: 1px solid rgba(16, 185, 129, 0.1); display: flex; flex-direction: column; gap: 15px; }",
            "        .kanban-header { font-weight: 800; text-transform: uppercase; letter-spacing: 1px; padding-bottom: 10px; border-bottom: 2px solid; }",
            "        .kanban-card { background: rgba(6, 78, 59, 0.8); border: 1px solid rgba(52, 211, 153, 0.2); padding: 15px; border-radius: 12px; display: flex; flex-direction: column; gap: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.3); transition: transform 0.2s; }",
            "        .kanban-card:hover { transform: translateY(-3px); border-color: rgba(52, 211, 153, 0.5); }",
            ...
            "        .top-navbar { display: flex; justify-content: space-between; align-items: center; padding: 15px 30px; background: rgba(2, 28, 20, 0.8); backdrop-filter: blur(20px); border-bottom: 1px solid var(--border-color); position: sticky; top: 0; z-index: 1000; }",
            "        .nav-brand { font-size: 1.8em; font-weight: 800; color: var(--accent-green); text-transform: uppercase; letter-spacing: 2px; text-shadow: 0 0 20px rgba(16, 185, 129, 0.4); display: flex; align-items: center; gap: 15px; }",
            "        .nav-menu { display: flex; gap: 10px; overflow-x: auto; padding: 15px 30px; background: rgba(2, 28, 20, 0.6); backdrop-filter: blur(10px); border-bottom: 1px solid var(--border-color); white-space: nowrap; }",
            "        .nav-menu::-webkit-scrollbar { height: 4px; } .nav-menu::-webkit-scrollbar-thumb { background: var(--accent-green); border-radius: 2px; }",
            "        .nav-btn { background: rgba(16, 185, 129, 0.05); border: 1px solid rgba(16, 185, 129, 0.1); color: var(--text-muted); padding: 10px 20px; border-radius: 12px; cursor: pointer; font-weight: 600; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: flex; align-items: center; gap: 8px; }",
            "        .nav-btn:hover { background: rgba(16, 185, 129, 0.15); color: var(--text-main); transform: translateY(-3px); box-shadow: 0 8px 20px rgba(16, 185, 129, 0.2); }",
            "        .nav-btn.active { background: linear-gradient(135deg, rgba(16, 185, 129, 0.2) 0%, rgba(16, 185, 129, 0.05) 100%); border-color: var(--accent-green); color: var(--accent-green); box-shadow: 0 8px 25px rgba(16, 185, 129, 0.3); transform: translateY(-3px); }",
            "        .main-content { padding: 30px; max-width: 1600px; margin: 0 auto; animation: fadeInSmooth 0.5s ease; overflow-y: auto; height: calc(100vh - 150px); }",
            "        ",
            "        /* CHAT UI REFINED */",
            "        .chat-container { display: flex; height: 650px; gap: 25px; }",
            "        .ticket-list { flex: 1; background: rgba(2, 44, 34, 0.4); border-radius: 20px; border: 1px solid rgba(16, 185, 129, 0.1); overflow-y: auto; padding: 15px; display: flex; flex-direction: column; gap: 10px; }",
            "        .ticket-item { padding: 15px; background: rgba(52, 211, 153, 0.05); border-radius: 12px; cursor: pointer; transition: all 0.3s; font-weight: 600; font-size: 0.9em; border: 1px solid transparent; }",
            "        .ticket-item:hover { background: rgba(52, 211, 153, 0.1); border-color: rgba(52, 211, 153, 0.2); }",
            "        .ticket-item.active { background: linear-gradient(90deg, rgba(16, 185, 129, 0.2) 0%, transparent 100%); border-left: 3px solid var(--accent-green); color: #fff; transform: translateX(5px); }",
            "        .chat-window { flex: 3; display: flex; flex-direction: column; background: rgba(2, 44, 34, 0.4); border-radius: 20px; border: 1px solid rgba(16, 185, 129, 0.1); overflow: hidden; position: relative; }",
            ...
            "        .chat-bubble.bot { align-self: flex-end; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #fff; border-bottom-right-radius: 4px; font-weight: 500; }",
            "        .chat-bubble.user { align-self: flex-start; background: rgba(52, 211, 153, 0.1); color: white; border-bottom-left-radius: 4px; border: 1px solid rgba(52, 211, 153, 0.2); }",
            "        .chat-author { font-size: 0.7em; opacity: 0.7; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 1px; font-weight: 800; }",
            "        .chat-input-area { display: flex; padding: 20px; background: rgba(2, 44, 34, 0.8); border-top: 1px solid rgba(16, 185, 129, 0.1); gap: 15px; align-items: center; backdrop-filter: blur(10px); }",
            "        .chat-input-area input[type='text'] { flex: 1; margin: 0; background: rgba(6, 78, 59, 0.6); border-radius: 12px; }",
            "        ",
            "        /* ULTRA PREMIUM TOAST */",
            "        #toast { position:fixed; bottom: 20px; right: 20px; background: rgba(2, 44, 34, 0.9); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); color: white; padding: 16px 24px; border-radius: 12px; font-weight: 600; font-size: 0.95em; display: flex; align-items: center; gap: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); border: 1px solid rgba(16, 185, 129, 0.3); transform: translateY(150px) scale(0.9); opacity: 0; transition: transform 0.5s cubic-bezier(0.22, 1, 0.36, 1), opacity 0.5s ease; z-index: 10000; pointer-events: none; }",
            ...
            "        .modal-content { background:var(--bg-main); padding:35px; border-radius:16px; border:1px solid var(--accent-green); text-align:center; max-width:400px; box-shadow: 0 10px 50px rgba(16, 185, 129, 0.3); animation: zoomIn 0.3s forwards; }",
            ...
            "       <h1 style='color:var(--accent-green); font-size: 3em; margin-bottom: 20px; letter-spacing: 4px; text-shadow: 0 0 20px rgba(16, 185, 129, 0.5);'>NEXUS CORE</h1>",
            "       <div class='loader-bar-fast'></div>",
            "    </div>",
            "",
            "    <div class='modal' id='premium-dialog' style='display:none; position:fixed; inset:0; background:rgba(2, 28, 20, 0.85); backdrop-filter:blur(20px); z-index:10001; justify-content:center; align-items:center;'>",
            "        <div class='modal-content' style='background:rgba(6, 78, 59, 0.8); padding:40px; border-radius:24px; border:1px solid rgba(16, 185, 129, 0.3); width:90%; max-width:450px; text-align:left; box-shadow:0 20px 60px rgba(0,0,0,0.8), inset 0 0 20px rgba(16, 185, 129, 0.1); animation:fadeInSmooth 0.4s ease-out;'>",
            "            <h2 id='dialog-title' style='margin:0 0 15px 0; color:var(--accent-green); font-size:1.4em; font-weight:800; letter-spacing:1px; text-transform:uppercase;'>Title</h2>",
            ...
            "                <button class='admin-btn' id='dialog-confirm' style='flex:1; margin:0; background:rgba(16, 185, 129, 0.2); border-color:var(--accent-green); color:var(--accent-green);'>Execute</button>",
            "            </div>",
            "        </div>",
            "    </div>",
            "",
            "    <header class='top-navbar'>",
            "        <div class='nav-brand'>NEXUS <span style='font-weight:300; color:var(--text-muted); margin-left:10px;'>ADMIN</span></div>",
            ...
            "        <button class='nav-btn' onclick='window.switchTab(\"products\", this)'>🛍️ Catalog</button>",
            ...
            "        <button class='nav-btn' onclick='window.switchTab(\"referrals\", this)'>🎟️ Promos</button>",
            ...
            "        <button class='nav-btn' onclick='window.switchTab(\"admin\", this)'>⚙️ Settings <span class='nav-badge' id='badge-admin'>0</span></button>",
            "    </nav>",
            "",
            ...
            "                   <div class='card green' onclick='window.editStat(\"total_rev\")' style='cursor:pointer;' title='Click to edit'><h3>Total Earnings</h3><div class='value money text-green' id='ui-total-rev'>€0</div></div>",
            "                   <div class='card green' onclick='window.editStat(\"conv_rate\")' style='cursor:pointer;' title='Click to edit'><h3>Conversion Rate</h3><div class='value text-green' id='ui-conv-rate'>0%</div></div>",
            "                   <div class='card green' onclick='window.editStat(\"online_total\")' style='cursor:pointer;' title='Click to edit'><h3>Online / Total</h3><div class='value text-green' id='ui-online-total'>0</div></div>",
            "                   <div class='card green' onclick='window.editStat(\"retention\")' style='cursor:pointer;' title='Click to edit'><h3>Retention Rate</h3><div class='value text-green' id='ui-retention'>0%</div></div>",
            "               </div>",
            "               <div class='stats-grid'>",
            "                   <div class='card green' onclick='window.editStat(\"tickets\")' style='cursor:pointer;' title='Click to edit'><h3>Tickets Opened</h3><div class='value text-green' id='ui-tickets-opened'>0</div></div>",
            "                   <div class='card green' onclick='window.editStat(\"dropoff\")' style='cursor:pointer;' title='Click to edit'><h3>Drop-off Rate</h3><div class='value text-green' id='ui-dropoff'>0%</div></div>",
            "                   <div class='card green' onclick='window.editStat(\"peak\")' style='cursor:pointer;' title='Click to edit'><h3>Peak Sales Hour</h3><div class='value text-green' id='ui-peak-hour'>N/A</div></div>",
            "               </div>",
            ...
            "                               <button class='admin-btn' style='margin:0; padding:8px 15px;' onclick='window.location.href=\"/api/export\"'>📥 Export</button>",
            "                               <button class='admin-btn' id='btn-chart-7' style='margin:0; padding:8px 15px;' onclick='window.updateSalesChart(7)'>7D</button>",
            "                               <button class='admin-btn' id='btn-chart-30' style='margin:0; padding:8px 15px; background:transparent;' onclick='window.updateSalesChart(30)'>30D</button>",
            "                               <button class='admin-btn' id='btn-chart-all' style='margin:0; padding:8px 15px; background:transparent;' onclick='window.updateSalesChart(0)'>ALL</button>",
            "                           </div>",
            "                       </div>",
            ...
            "                       <div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid rgba(52, 211, 153, 0.2); padding-bottom:15px;'><h2 style='margin:0; border:none;'>⚡ Live Pulse</h2><div class='status-dot' style='margin:0;'></div></div>",
            ...
            "               <div class='box'>",
            "                   <h2 style='color:var(--accent-green);'>📋 Gestion des Commandes Personnalisées</h2>",
            ...
            "               <div class='box' style='background:rgba(16, 185, 129, 0.05); border-color:rgba(16, 185, 129, 0.3);'>",
            "                   <h2 style='color:var(--accent-green);'>👑 VIP Directory</h2>",
            ...
            "                       <button class='admin-btn' style='margin:0; background:var(--accent-green); color:#000;' onclick='window.loadTicketsForChat()'>🔄 Synchronise</button>",
            "                   </div>",
            ...
            "                               </div>",
            "                           </div>",
            "                           <div style='display:flex; gap:10px; padding: 15px; background: rgba(2, 44, 34, 0.8); border-top: 1px solid rgba(16, 185, 129, 0.2); flex-wrap: wrap;'>",
            "                               <button class='admin-btn' style='margin:0; padding:8px 15px;' onclick='window.sendQuickResponse(\"welcome\")'>👋 Welcome</button>",
            "                               <button class='admin-btn' style='margin:0; padding:8px 15px;' onclick='window.sendQuickResponse(\"wait\")'>⏳ Wait</button>",
            "                               <button class='admin-btn' style='margin:0; padding:8px 15px;' onclick='window.sendQuickResponse(\"resolved\")'>✅ Resolved?</button>",
            ...
            "               <div class='box' style='border-color: var(--accent-green); box-shadow: 0 0 20px rgba(16,185,129,0.1);'>",
            "                   <h2 style='color: var(--accent-green);'>➕ Ajouter une Transaction Manuelle</h2>",
            "                   <div style='display:flex; gap:15px; flex-wrap:wrap; margin-bottom:15px;'>",
            "                       <input type='text' id='manTxUser' placeholder='Username Discord' style='flex:1; min-width:150px;'>",
            "                       <input type='text' id='manTxProd' placeholder='Produit Choisi' style='flex:1; min-width:150px;'>",
            "                       <input type='number' id='manTxPrice' placeholder='Prix (€)' style='width:120px;'>",
            "                       <input type='datetime-local' id='manTxDate' style='width:200px;'>",
            "                   </div>",
            "                   <button class='admin-btn' style='background:var(--accent-green); color:#000; width:100%;' onclick='window.createManualTx()'>🚀 Enregistrer la Transaction</button>",
            "               </div>",
            ...
            "                   </div>",
            "                   <div style='display:flex; gap:20px; flex-wrap:wrap; margin-bottom:15px; padding:15px; border-radius:12px; background:rgba(16,185,129,0.05); border:1px solid rgba(16,185,129,0.2);'>",
            "                       <strong style='color:var(--accent-green); display:flex; align-items:center;'>🚀 Auto-Upsell :</strong>",
            ...
            "                       <button class='admin-btn' style='margin:0; background:var(--accent-green); color:#000; box-shadow:0 5px 15px rgba(16,185,129,0.3); padding:10px 20px;' onclick='window.triggerShopRefresh()'>🚀 PUSH MENU TO DISCORD</button>",
            "                   </div>",
            ...
            "                        <div class='card' id='card-discord'><h3>Discord WebSocket</h3><div class='value text-green' id='ui-discord-ws' style='font-size:1.5em;'>-- ms</div><p class='text-muted' style='margin-top:10px;'>Gateway Ping</p></div>",
            "                    </div>",
            "                    <div style='margin-top:40px; background:rgba(16, 185, 129, 0.05); padding:30px; border-radius:20px; border:1px solid rgba(16, 185, 129, 0.2);'>",
            "                        <h3 style='color:var(--accent-green); margin-top:0;'>⚡ End-to-End Latency Vector</h3>",
            "                        <p class='text-muted'>Measures round-trip execution speed from dashboard click to Discord network parsing.</p>",
            "                        <div style='display:flex; align-items:center; gap:30px; margin-top:25px;'><button class='admin-btn' style='margin:0; padding:15px 30px;' onclick='window.testActionLatency()'>⚡ Execute Ping</button><div id='latency-result' style='font-size:2em; font-weight:800; color:var(--text-main); font-family:monospace;'>-- ms</div></div>",
            "                    </div>",
            "                </div>",
            "            </div>",
            "                ",
            ...
            "                <div class='box' style='border:1px solid rgba(16, 185, 129, 0.2); background:rgba(16, 185, 129, 0.05);'>",
            "                    <h2 style='color:var(--accent-green); margin-top:0; border-bottom-color:rgba(16, 185, 129, 0.1);'>⏳ Review Queue</h2>",
            ...
            "                    <button class='admin-btn' style='width:100%; padding:15px;' onclick='window.sendReview()'>📤 Broadcast to Network</button>",
            "                </div>",
            "            </div>",
            ...
            "              <div class='kanban-col'><div class='kanban-header text-blue' style='border-color:var(--accent-green)'>📬 NOUVELLES DEMANDES</div>\${kPending||'<p class=\"text-muted\">Vide</p>'}</div>",
            ...
            "                        historyHtml += '<div style=\"font-size:0.85em; margin-bottom:5px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:5px;\"><span style=\"color:var(--accent-green);\">🛒 ' + escapeHTML(h.product) + '</span> - €' + h.price + ' <br><span style=\"opacity:0.5; font-size:0.9em;\">' + h.date + '</span></div>'; ",
            "                    }); ",
            "                } else historyHtml = '<span class=\"text-muted\" style=\"font-family:monospace;\">No purchases</span>'; ",
            "                html += '<div class=\"card\" style=\"margin-bottom: 25px; border-left: 4px solid ' + trustColor + '; background:rgba(6, 78, 59, 0.4);\">' +",
            ...
            "                                '<label style=\"font-size:0.8rem; font-weight:600; text-transform:uppercase; letter-spacing:1px; color:var(--accent-green); display:block; margin-bottom:10px;\">📝 Admin Directives:</label>' +",
            "                                '<textarea id=\"note-' + m.id + '\" placeholder=\"Inject private private parameters...\" style=\"min-height:60px; background:rgba(0,0,0,0.5);\" onblur=\"window.saveUserNote(\\'' + m.id + '\\')\">' + safeNote + '</textarea>' +",
            "                            '</div>' +",
            ...
            "                        '<button class=\"admin-btn\" style=\"margin:0; background:rgba(16,185,129,0.1); border-color:var(--accent-green); color:var(--accent-green);\" onclick=\"window.openDirectContact(\\'' + m.id + '\\')\">💬 Inject DM</button>' +",
            ...
            "        ",
            "        window.loadTicketsForChat = async function() { try { const res = await fetch('/api/tickets'); const tickets = await res.json(); let html = ''; if(tickets.length === 0) { html = '<p class=\"text-muted text-center\" style=\"margin-top:20px; font-family:monospace;\">No active lines.</p>'; } else { const shopTickets = tickets.filter(t => t.name.startsWith('shop-')); const supportTickets = tickets.filter(t => t.name.startsWith('support-')); if(shopTickets.length > 0) { html += '<div style=\"font-size:0.85em; text-transform:uppercase; color:var(--accent-green); font-weight:800; margin: 10px 0 5px 5px; border-bottom: 1px solid rgba(16, 185, 129, 0.2); padding-bottom:3px;\">🛒 Shop (' + shopTickets.length + ')</div>'; shopTickets.forEach(t => { const isActive = activeChatChannel === t.id ? 'active' : ''; html += '<div class=\"ticket-item ' + isActive + '\" onclick=\"window.openTicketChat(\\'' + t.id + '\\')\">🛒 ' + escapeHTML(t.name) + '</div>'; }); } if(supportTickets.length > 0) { html += '<div style=\"font-size:0.85em; text-transform:uppercase; color:var(--accent-orange); font-weight:800; margin: 15px 0 5px 5px; border-bottom: 1px solid rgba(245, 158, 11, 0.2); padding-bottom:3px;\">🎧 Support (' + supportTickets.length + ')</div>'; supportTickets.forEach(t => { const isActive = activeChatChannel === t.id ? 'active' : ''; html += '<div class=\"ticket-item ' + isActive + '\" onclick=\"window.openTicketChat(\\'' + t.id + '\\')\">🎧 ' + escapeHTML(t.name) + '</div>'; }); } } document.getElementById('chat-ticket-list').innerHTML = html; } catch(e) {} };",
            "        window.openTicketChat = function(channelId) { activeChatChannel = channelId; window.loadTicketsForChat(); document.getElementById('chat-messages-area').innerHTML = '<div style=\"margin:auto; color:var(--accent-green);\"><div style=\"width:40px; height:40px; border:3px solid rgba(16, 185, 129, 0.1); border-top:3px solid var(--accent-green); border-radius:50%; animation:spin 1s linear infinite; margin:auto; box-shadow:0 0 15px rgba(16, 185, 129, 0.5);\"></div></div>'; window.fetchChatMessages(); if(chatPollInterval) clearInterval(chatPollInterval); chatPollInterval = setInterval(window.fetchChatMessages, 3000); };",
