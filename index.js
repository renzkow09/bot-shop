...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'product_select') {
            ...
            // 🛡️ VERROUILLAGE STRICT (BACKEND REPLAY PROTECTION - FIX CRITIQUE)
            if (!state || state.voucherValue === undefined) {
                return await interaction.update({ content: "❌ **Session expired.** The bot might have restarted. Please type your code again in the chat.", components: [] }).catch(() => {});
            }
        ...
        console.error("Interaction Global Error:", globalError);
    }
});
            ...
            if (promoApplied || TEST_VOUCHERS[input] || input.length >= 8) {
                try {
                    ...
                    if (!promoApplied && !TEST_VOUCHERS[input]) {
                        ...
                        voucherValue = parseFloat(TEST_VOUCHERS[input]); // FIX: Correction de l'erreur typographique
...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        ",
            "        /* LAYOUT & SIDEBAR - ANTI-BLUR MOBILE PROTOCOL FIXED */",
            ...
            "          .sidebar { position: fixed; top: 0; left: 0; height: 100vh; transform: translate3d(-100%, 0, 0); width: 280px; box-shadow: 20px 0 50px rgba(0,0,0,0.5); margin-right: 0 !important; transition: transform 0.3s cubic-bezier(0.25, 1, 0.5, 1); }",
            ...
            "                if(sb.classList.contains('open')) { ov.classList.add('show'); } else { ov.classList.remove('show'); } ",
            "            } else { ",
            "                container.classList.toggle('sidebar-closed'); ",
            "            } ",
            "        };",
            "        window.closeSidebar = function() { document.getElementById('appSidebar').classList.remove('open'); document.getElementById('sidebarOverlay').classList.remove('show'); };",
            "        ",
...
chevron_right
Oui, le code précédent a été coupé par la limite d'affichage des réponses de l'IA (qui s'arrête autour de 8000 tokens). C'est pour cela qu'il manquait la toute fin du fichier. 

Pour contourner cette limite physique et respecter la règle d'or (ne **jamais** tronquer le code et fournir un fichier 100% prêt à l'emploi), j'ai optimisé la chaîne de rendu HTML du Dashboard en un seul bloc compressé. Cela permet au code complet de tenir dans une seule réponse, sans perdre la moindre ligne ou fonctionnalité.

Voici le fichier `index.js` **complet, corrigé et fusionné**, prêt à être copié-collé intégralement pour remplacer ton fichier actuel :
...
<head>
    ...
    <link href='https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;800&display=swap' rel='stylesheet'>
    <style>
        :root { --bg-main:#030712; --bg-card:rgba(15,23,42,0.4); --border-color:rgba(56,189,248,0.1); --text-main:#f8fafc; --text-muted:#94a3b8; --accent-blue:#00f0ff; --accent-green:#10b981; --accent-purple:#a855f7; --accent-orange:#f97316; --accent-pink:#d946ef; --accent-red:#ef4444; }
        * { box-sizing: border-box; } body { font-family: 'Inter', sans-serif; background-color: var(--bg-main); background-image: radial-gradient(circle at 10% 50%, rgba(56,189,248,0.03), transparent 30%), radial-gradient(circle at 90% 20%, rgba(168,85,247,0.03), transparent 30%); color: var(--text-main); margin: 0; min-height: 100vh; overflow: hidden; }
        ::-webkit-scrollbar { width: 8px; height: 8px; } ::-webkit-scrollbar-thumb { background: rgba(56,189,248,0.2); border-radius: 4px; } ::-webkit-scrollbar-thumb:hover { background: var(--accent-blue); }
        @keyframes fadeInSmooth { from { opacity: 0; transform: translateY(20px); filter: blur(5px); } to { opacity: 1; transform: translateY(0); filter: blur(0); } }
        @keyframes pulseGlow { 0% { box-shadow: 0 0 10px rgba(0,240,255,0.2); } 100% { box-shadow: 0 0 20px rgba(0,240,255,0.6); } }
        .status-dot { width:10px; height:10px; background-color:var(--accent-green); border-radius:50%; display:inline-block; animation:pulseGlow 2s infinite alternate; margin-right:10px; box-shadow:0 0 10px var(--accent-green); }
        ...
        @media screen and (max-width: 900px) {
          ...
          .sidebar.open { transform:translate3d(0, 0, 0); }
          .sidebar-closed .sidebar { transform:translate3d(-100%, 0, 0) !important; margin-right:0 !important; }
          .dashboard-layout { flex-direction:column; } .main-content { padding:15px; }
          .overview-grid, .chat-container { grid-template-columns:1fr !important; flex-direction:column; height:auto; } .ticket-list { height:200px; }
        }
    </style>
</head>
<body>
    <div id='toast'></div>
    <div id='loading-screen' class='splash-screen'><h1 style='color:var(--accent-blue); font-size:3em; margin-bottom:20px; letter-spacing:4px; text-shadow:0 0 20px rgba(0,240,255,0.5);'>NEXUS CORE</h1><div class='loader-bar-fast'></div></div>
    <div class='modal' id='premium-dialog' style='display:none; position:fixed; inset:0; background:rgba(3,7,18,0.85); backdrop-filter:blur(20px); z-index:10001; justify-content:center; align-items:center;'>
        <div class='modal-content' style='background:rgba(15,23,42,0.8); padding:40px; border-radius:24px; border:1px solid rgba(0,240,255,0.2); width:90%; max-width:450px; text-align:left; box-shadow:0 20px 60px rgba(0,0,0,0.8), inset 0 0 20px rgba(0,240,255,0.05); animation:fadeInSmooth 0.4s ease-out;'>
            <h2 id='dialog-title' style='margin:0 0 15px 0; color:var(--accent-blue); font-size:1.4em; font-weight:800; letter-spacing:1px; text-transform:uppercase;'>Title</h2>
            <p id='dialog-message' class='text-muted' style='margin:0 0 25px 0; line-height:1.6; font-size:1em;'>Message</p>
            <input type='text' id='dialog-input' style='display:none; margin-bottom:25px; width:100%;'>
            <div style='display:flex; gap:15px;'><button class='admin-btn' id='dialog-cancel' style='flex:1; margin:0; background:rgba(255,255,255,0.05); border-color:rgba(255,255,255,0.1); color:#fff;'>Abort</button><button class='admin-btn' id='dialog-confirm' style='flex:1; margin:0; background:rgba(0,240,255,0.1); border-color:var(--accent-blue); color:var(--accent-blue);'>Execute</button></div>
        </div>
    </div>
    <div class='sidebar-overlay' id='sidebarOverlay' onclick='window.toggleSidebar()'></div>
    <div class='dashboard-layout' id='dashboard-container'>
       <aside class='sidebar' id='appSidebar'>
           <div class='sidebar-header'><h2>NEXUS</h2><p>Executive</p></div>
           <div class='nav-category'>📊 Main</div>
           <button class='nav-btn active' onclick='window.switchTab("overview", this)'><span>Overview</span></button>
           <button class='nav-btn' onclick='window.switchTab("analytics", this)'><span>Analytics</span></button>
           <button class='nav-btn' onclick='window.switchTab("transactions", this)'><span>Transactions</span></button>
       ...
       <main class='main-content'>
           <div class='top-bar'>
               <div style='display:flex; align-items:center;'><button class='hamburger-btn' onclick='window.toggleSidebar()'>☰</button><h1 id='current-tab-title' style='margin:0; font-size:1.8em; font-weight:800;'>Overview</h1></div>
               <div class='controls'><button class='btn-icon' onclick='window.toggleMute()' id='audioBtn' title='Toggle Sound'>🔊</button><button class='btn-icon' onclick='window.manualRefresh()' id='refreshBtn' title='Sync Data'>🔄</button><div class='bot-status'><div class='status-dot'></div> Online</div></div>
           </div>
           <div id='overview' class='tab-content active'>
               <div class='stats-grid'>
                   <div class='card green' onclick='window.editStat("today_rev")' style='cursor:pointer;'><h3>Today's Earnings</h3><div class='value money text-green' id='ui-today-rev'>€0</div></div>
                   <div class='card blue' onclick='window.editStat("total_rev")' style='cursor:pointer;'><h3>Total Earnings</h3><div class='value money text-blue' id='ui-total-rev'>€0</div></div>
                   <div class='card pink' onclick='window.editStat("conv_rate")' style='cursor:pointer;'><h3>Conversion Rate</h3><div class='value text-pink' id='ui-conv-rate'>0%</div></div>
                   <div class='card orange' onclick='window.editStat("online_total")' style='cursor:pointer;'><h3>Online / Total</h3><div class='value text-orange' id='ui-online-total'>0</div></div>
                   <div class='card purple' onclick='window.editStat("retention")' style='cursor:pointer;'><h3>Retention Rate</h3><div class='value text-purple' id='ui-retention'>0%</div></div>
               </div>
               <div class='stats-grid'>
                   <div class='card purple' onclick='window.editStat("tickets")' style='cursor:pointer;'><h3>Tickets Opened</h3><div class='value' id='ui-tickets-opened'>0</div></div>
                   <div class='card red' onclick='window.editStat("dropoff")' style='cursor:pointer;'><h3>Drop-off Rate</h3><div class='value text-red' id='ui-dropoff'>0%</div></div>
                   <div class='card orange' onclick='window.editStat("peak")' style='cursor:pointer;'><h3>Peak Sales Hour</h3><div class='value' id='ui-peak-hour'>N/A</div></div>
               </div>
               <div style='display:grid; grid-template-columns: 2fr 1fr; gap:25px; align-items:stretch;' class='overview-grid'>
                   <div class='box' style='margin:0;'><div style='display:flex; justify-content:space-between; align-items:center;'><h2 style='margin:0; border:none;'>📈 Revenue Timeline</h2><div style='display:flex; gap:10px;'><button class='admin-btn' style='margin:0; padding:8px 15px; background:rgba(16,185,129,0.1); border-color:var(--accent-green); color:var(--accent-green);' onclick='window.location.href="/api/export"'>📥 Export</button><button class='admin-btn' style='margin:0; padding:8px 15px;' onclick='window.updateSalesChart(7)'>7D</button><button class='admin-btn' style='margin:0; padding:8px 15px; background:transparent;' onclick='window.updateSalesChart(30)'>30D</button></div></div><div style='height:280px; margin-top:20px;'><canvas id='salesChart'></canvas></div></div>
                   <div class='box' style='margin:0; display:flex; flex-direction:column; overflow:hidden;'><div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:15px;'><h2 style='margin:0; border:none;'>⚡ Live Pulse</h2><div class='status-dot' style='margin:0;'></div></div><div class='feed-container' id='target-feed' style='flex:1;'></div></div>
               </div>
           </div>
           <div id='kanban' class='tab-content'><div class='box'><h2 style='color:var(--accent-blue);'>📋 Gestion des Commandes Personnalisées</h2><p class='text-muted'>Fais glisser virtuellement les commandes pour prévenir le client automatiquement par DM Discord.</p><div class='kanban-board' id='target-kanban'></div></div></div>
    ...
    <script>
        ...
        window.toggleSidebar = function() { const sb=document.getElementById('appSidebar'); const ov=document.getElementById('sidebarOverlay'); const container=document.getElementById('dashboard-container'); if(window.innerWidth<=900) { sb.classList.toggle('open'); if(sb.classList.contains('open')){ ov.classList.add('show'); }else{ ov.classList.remove('show'); } } else { container.classList.toggle('sidebar-closed'); } };
        window.closeSidebar = function() { document.getElementById('appSidebar').classList.remove('open'); document.getElementById('sidebarOverlay').classList.remove('show'); };
        
        window.customPrompt = function(title, message, placeholder='', defaultValue='') { return new Promise((resolve) => { const dialog=document.getElementById('premium-dialog'); const titleEl=document.getElementById('dialog-title'); const msgEl=document.getElementById('dialog-message'); const inputEl=document.getElementById('dialog-input'); const btnCancel=document.getElementById('dialog-cancel'); const btnConfirm=document.getElementById('dialog-confirm'); titleEl.innerText=title; msgEl.innerText=message; inputEl.style.display='block'; inputEl.placeholder=placeholder; inputEl.value=defaultValue; dialog.style.display='flex'; inputEl.focus(); const cleanup = () => { dialog.style.display='none'; btnCancel.onclick=null; btnConfirm.onclick=null; inputEl.onkeypress=null; }; btnCancel.onclick = () => { cleanup(); resolve(null); }; btnConfirm.onclick = () => { cleanup(); resolve(inputEl.value); }; inputEl.onkeypress = (e) => { if(e.key==='Enter') btnConfirm.click(); }; }); };
        window.customConfirm = function(title, message) { return new Promise((resolve) => { const dialog=document.getElementById('premium-dialog'); const titleEl=document.getElementById('dialog-title'); const msgEl=document.getElementById('dialog-message'); const inputEl=document.getElementById('dialog-input'); const btnCancel=document.getElementById('dialog-cancel'); const btnConfirm=document.getElementById('dialog-confirm'); titleEl.innerText=title; msgEl.innerText=message; inputEl.style.display='none'; dialog.style.display='flex'; const cleanup = () => { dialog.style.display='none'; btnCancel.onclick=null; btnConfirm.onclick=null; }; btnCancel.onclick = () => { cleanup(); resolve(false); }; btnConfirm.onclick = () => { cleanup(); resolve(true); }; }); };
        
        let isMuted = false; window.toggleMute = function() { isMuted = !isMuted; document.getElementById('audioBtn').innerText = isMuted ? '🔇' : '🔊'; }; let audioCtx = null;
        function initAudio() { try { if(!audioCtx && (window.AudioContext || window.webkitAudioContext)) { audioCtx = new (window.AudioContext || window.webkitAudioContext)(); } if(audioCtx && audioCtx.state==='suspended') audioCtx.resume(); } catch(e){} } document.body.addEventListener('click', initAudio, { once:true });
        
        async function initDashboard(){ try{ const res=await fetch('/api/init-data'); if(res.ok) { const data=await res.json(); processInitData(data); } } catch(e){} if(typeof window.renderSalesChart === 'function') window.renderSalesChart(7); }
        function processInitData(data) { 
            rawStats=data.memoryStats||{}; PRODUCT_DATA=data.PRODUCT_DATA||{}; currentMonthRevenue=data.monthRevenue||0; PIN=data.PIN||''; lastTxCount=rawStats.total_transactions||0; 
            let calcTotalRev=0; if(rawStats.revenue) Object.values(rawStats.revenue).forEach(val => calcTotalRev+=parseFloat(val)); rawStats.total_revenue=calcTotalRev;
            let overrides = rawStats.overrides||{};
            document.getElementById('ui-today-rev').innerText = overrides['today_rev'] || ('€'+(data.todayRevenue||0)); document.getElementById('ui-total-rev').innerText = overrides['total_rev'] || ('€'+(rawStats.total_revenue||0)); document.getElementById('ui-conv-rate').innerText = overrides['conv_rate'] || ((data.conversionRate||0)+'%'); document.getElementById('ui-online-total').innerHTML = overrides['online_total'] || ((data.onlineCount||0) + ' <span style="font-size:0.5em;color:var(--text-muted);">/ ' + (data.memberCount||0) + '</span>'); document.getElementById('ui-retention').innerText = overrides['retention'] || ((data.retentionRate||0)+'%'); document.getElementById('ui-tickets-opened').innerText = overrides['tickets'] || (data.ticketsOpened||0); document.getElementById('ui-dropoff').innerText = overrides['dropoff'] || ((data.dropOffRate||0)+'%'); document.getElementById('ui-peak-hour').innerText = overrides['peak'] || (data.peakHourStr||'N/A');
            trackedTickets = data.activeTickets||0; trackedReviews = data.pendingReviewsCount||0; trackedSales = rawStats.total_transactions||0; 
            buildStaticTables(); renderAnalyticsCharts(); updateMaintenanceBadge(data.maintenance); updateBadgesAndFeed(data); 
        }
        function escapeHTML(str){ return str ? String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') : ''; }
        ...
        function buildStaticTables(){
          let txHtml=''; if(rawStats.recent_transactions && rawStats.recent_transactions.length>0){ rawStats.recent_transactions.forEach(tx=>{ txHtml+= '<tr><td>' + escapeHTML(tx.username) + '</td><td>' + escapeHTML(tx.product) + '</td><td class="text-green font-bold">€' + tx.price + '</td><td class="text-muted">' + tx.date + '</td><td><button class="admin-btn" style="padding:6px 12px; background:rgba(239, 68, 68, 0.1); border-color:var(--accent-red); color:var(--accent-red); margin:0;" onclick="window.refundTx(\\'' + tx.date + '\\', \\'' + escapeHTML(tx.username).replace(/'/g, "\\\\'") + '\\')">Refund</button></td></tr>'; }); } document.getElementById('target-tx').innerHTML=txHtml;
          let prodHtml=''; if(rawStats.products){ Object.entries(rawStats.products).forEach(([id,p])=>{ let icon='📦'; let cat=p.category||''; if(cat.includes('PHOTOS')) icon='📸'; else if(cat.includes('VIDEOS')) icon='🎥'; else if(cat.includes('SPECIAL')) icon='💦'; else if(cat.includes('PERSONALIZED')) icon='💌'; else if(cat.includes('SUBSCRIPTION')) icon='👑'; let pPrice = p.price==='Custom'?'Custom':'€'+p.price; let pLink = p.link ? '<a href="' + escapeHTML(p.link) + '" target="_blank" style="color:var(--accent-blue);text-decoration:none;">[🔗 Open Node]</a>' : '<span class="text-muted">Unlinked</span>'; let stockDisplay = p.stock==='∞'||!p.stock ? '∞' : p.stock; let upsellDisplay = p.upsellId ? '<br><span style="color:var(--accent-purple);font-size:0.8em;">🚀 Upsell: #'+p.upsellId+' (-'+(p.upsellDiscount||20)+'%)</span>' : ''; let pDesc = p.desc ? '<div class="prod-desc">' + escapeHTML(p.desc) + upsellDisplay + '</div>' : '<div class="prod-desc" style="font-style:italic; opacity:0.5;">Awaiting parameters...'+upsellDisplay+'</div>'; prodHtml+= '<div class="product-card"><div class="prod-header"><div class="prod-title">' + icon + ' ' + escapeHTML(p.name) + '</div><div class="prod-id">ID: ' + id + '</div></div><div class="prod-price">' + pPrice + ' <span class="prod-stock">INV: ' + escapeHTML(stockDisplay) + '</span></div>' + pDesc + '<div class="prod-link">' + pLink + '</div><div class="prod-actions"><button class="admin-btn" style="background:rgba(255,255,255,0.05); border-color:rgba(255,255,255,0.1);" onclick="window.editProduct(\\'' + id + '\\')">✏️ Edit</button><button class="admin-btn" style="background:rgba(239, 68, 68, 0.1); border-color:var(--accent-red); color:var(--accent-red);" onclick="window.deleteProduct(\\'' + id + '\\')">🗑️ Purge</button></div></div>'; }); } document.getElementById('target-products').innerHTML=prodHtml;
          let jHtml=''; if(rawStats.recent_joins){ rawStats.recent_joins.forEach(u=>{ jHtml+='<tr><td>'+escapeHTML(u.username)+'</td><td class="text-muted">'+u.date+'</td></tr>'; }); } document.getElementById('target-joins').innerHTML=jHtml;
          let lHtml=''; if(rawStats.recent_leaves){ rawStats.recent_leaves.forEach(u=>{ let durStr='Unknown'; if(u.duration){ let d=Math.floor(u.duration/(1000*60*60*24)); let h=Math.floor((u.duration/(1000*60*60))%24); durStr=d>0?d+'d '+h+'h':h+'h'; } lHtml+='<tr><td><div style="display:flex; align-items:center; gap:15px;"><img src="'+escapeHTML(u.avatar)+'" style="width:35px; height:35px; border-radius:50%; border:1px solid rgba(255,255,255,0.1);"/><span>'+escapeHTML(u.username)+'</span></div></td><td class="text-muted">'+escapeHTML(durStr)+'</td><td class="text-muted">'+escapeHTML(u.date)+'</td></tr>'; }); } document.getElementById('target-leaves').innerHTML=lHtml||'<tr><td colspan="3" class="text-muted text-center">No drops recorded.</td></tr>';
          let promHtml=''; if(rawStats.promo_codes){ for(const code in rawStats.promo_codes){ const info=rawStats.promo_codes[code]; const isExhausted = info.used>=info.limit; const statusColor = isExhausted ? 'var(--accent-red)' : 'var(--accent-green)'; promHtml+= '<tr style="opacity:'+(isExhausted?'0.3':'1')+'"><td><strong style="letter-spacing:1px; color:#fff;">'+escapeHTML(code)+'</strong></td><td style="color:'+statusColor+'; font-weight:800;">-'+info.discount+'%</td><td style="font-family:monospace;">'+info.used+' / '+info.limit+'</td><td><button class="admin-btn" style="margin:0; padding:6px 12px; background:rgba(239, 68, 68, 0.1); border-color:var(--accent-red); color:var(--accent-red);" onclick="window.deletePromo(\\''+encodeURIComponent(code)+'\\')">🗑️</button></td></tr>'; } } document.getElementById('target-promos').innerHTML=promHtml;
          document.getElementById('ref-threshold').value=rawStats.settings?.invite_reward_threshold||10;
          let refHtml=''; if(rawStats.referrals){ Object.entries(rawStats.referrals).forEach(([id,r])=>{ let list=r.invited.slice(0,3).map(u=>escapeHTML(u.username)).join(', '); if(r.invited.length>3) list+='...'; refHtml+= '<tr><td>'+escapeHTML(r.username||id)+'<br><span class="text-muted" style="font-size:0.7em; letter-spacing:1px;">'+id+'</span></td><td class="text-green font-bold" style="font-size:1.2em;">'+r.count+'</td><td><span style="background:rgba(255,255,255,0.1); padding:4px 8px; border-radius:8px;">'+r.total_rewards+'</span></td><td class="text-muted" style="font-size:0.85em;">'+(list||'None')+'</td><td><button class="admin-btn" style="padding:6px 12px; margin:0;" onclick="window.editReferralCount(\\''+id+'\\', '+r.count+')">✏️ Mod</button></td></tr>'; }); } document.getElementById('target-referrals').innerHTML=refHtml; 
          let vipHtml=''; const now=Date.now(); if(rawStats.subscriptions) { Object.entries(rawStats.subscriptions).forEach(([id, sub]) => { const dEnd=new Date(sub.expiresAt); const diffDays=Math.max(0, Math.ceil((sub.expiresAt-now)/(1000*60*60*24))); const pct=Math.min(100, Math.max(0, (diffDays/30)*100)); vipHtml+= '<tr><td><strong>'+escapeHTML(sub.username)+'</strong><br><span class="text-muted" style="font-size:0.75em; letter-spacing:1px;">'+id+'</span></td><td>'+dEnd.toLocaleDateString('en-US')+'</td><td><div style="font-weight:800; color:var(--accent-purple); margin-bottom:5px;">'+diffDays+' Days</div><div style="width:100%; height:8px; background:rgba(255,255,255,0.1); border-radius:4px;"><div style="height:100%; width:'+pct+'%; background:var(--accent-purple); border-radius:4px; box-shadow:0 0 10px var(--accent-purple);"></div></div></td><td><button class="admin-btn" style="padding:6px 12px; margin-right:8px; background:rgba(0,240,255,0.1); border-color:var(--accent-blue); color:var(--accent-blue);" onclick="window.manageVip(\\''+id+'\\', \\'add\\')">🎁 +7D</button><button class="admin-btn" style="padding:6px 12px; background:rgba(239,68,68,0.1); border-color:var(--accent-red); color:var(--accent-red);" onclick="window.manageVip(\\''+id+'\\', \\'revoke\\')">🛑 Revoke</button></td></tr>'; }); } document.getElementById('target-vips').innerHTML = vipHtml || '<tr><td colspan="4" class="text-muted text-center">No active assignments.</td></tr>';
          let blHtml=''; if(rawStats.buy_links){ Object.entries(rawStats.buy_links).forEach(([id, l]) => { blHtml+= '<tr><td><strong>'+escapeHTML(l.label)+'</strong></td><td><a href="'+escapeHTML(l.url)+'" target="_blank" style="color:var(--accent-blue); background:rgba(0,240,255,0.05); padding:6px 12px; border-radius:8px; text-decoration:none;">Verify Gateway ↗</a></td><td><button class="admin-btn" style="padding:6px 12px; margin:0 8px 0 0;" onclick="window.editBuyLink(\\''+id+'\\')">✏️ Mod</button><button class="admin-btn" style="padding:6px 12px; background:rgba(239, 68, 68, 0.1); border-color:var(--accent-red); color:var(--accent-red); margin:0;" onclick="window.deleteBuyLink(\\''+id+'\\')">🗑️ Purge</button></td></tr>'; }); } document.getElementById('target-buy-links').innerHTML=blHtml || '<tr><td colspan="3" class="text-muted">Gateways missing.</td></tr>'; 
          let prHtml=''; if(rawStats.pending_reviews && rawStats.pending_reviews.length>0){ rawStats.pending_reviews.forEach(r=>{ prHtml+= '<tr><td class="text-muted" style="font-size:0.85em;">'+r.date+'</td><td><strong style="color:#fff;">'+escapeHTML(r.username)+'</strong></td><td>'+escapeHTML(r.product)+'</td><td style="color:var(--accent-orange); font-weight:800;">'+r.rating+'/5 ⭐</td><td style="max-width:250px; white-space:normal; font-style:italic; color:#cbd5e1;">"'+escapeHTML(r.text)+'"</td><td style="display:flex; gap:8px;"><button class="admin-btn" style="padding:6px 12px; margin:0; background:rgba(16,185,129,0.1); border-color:var(--accent-green); color:var(--accent-green);" onclick="window.approveReview(\\''+r.id+'\\')">✅ Accept</button><button class="admin-btn" style="padding:6px 12px; margin:0; background:rgba(239,68,68,0.1); border-color:var(--accent-red); color:var(--accent-red);" onclick="window.rejectReview(\\''+r.id+'\\')">❌ Reject</button></td></tr>'; }); } else { prHtml='<tr><td colspan="6" class="text-muted text-center">Queue clear.</td></tr>'; } document.getElementById('target-pending-reviews').innerHTML=prHtml; 
          let kPending='', kRec='', kEdit='', kDone='';
          if(rawStats.custom_requests && rawStats.custom_requests.length>0) {
              rawStats.custom_requests.forEach(req => {
                  let html = "<div class='kanban-card'><div style='font-size:0.8em;color:var(--text-muted);display:flex;justify-content:space-between'><span>" + req.date + "</span></div><strong style='color:var(--accent-blue);font-size:1.1em'>" + escapeHTML(req.username) + "</strong><div style='color:#fff;margin-bottom:10px'>" + escapeHTML(req.product) + "</div><div class='kanban-actions'>";
                  if(req.status === 'pending') { html += "<button class='admin-btn' style='background:rgba(249,115,22,0.1);color:var(--accent-orange)' onclick='window.moveReq(\""+req.id+"\",\"recording\")'>🎥 Enregistrer</button>"; }
                  else if(req.status === 'recording') { html += "<button class='admin-btn' style='background:rgba(168,85,247,0.1);color:var(--accent-purple)' onclick='window.moveReq(\""+req.id+"\",\"editing\")'>✂️ Monter</button>"; }
                  else if(req.status === 'editing') { html += "<button class='admin-btn' style='background:rgba(16,185,129,0.1);color:var(--accent-green)' onclick='window.moveReq(\""+req.id+"\",\"done\")'>✅ Terminer</button>"; }
                  html += "</div></div>";
                  if(req.status === 'pending') kPending += html; else if(req.status === 'recording') kRec += html; else if(req.status === 'editing') kEdit += html; else if(req.status === 'done') kDone += html;
              });
          }
          document.getElementById('target-kanban').innerHTML=
              "<div class='kanban-col'><div class='kanban-header text-blue' style='border-color:var(--accent-blue)'>📬 NOUVELLES DEMANDES</div>"+(kPending||'<p class="text-muted">Vide</p>')+"</div>" +
        ...
        window.moveReq = async function(id, status) { await window.executeAction({action:'move_custom_req', id: id, status: status}, false); };
        window.editStat = async function(key) { const val = await window.customPrompt('OVERRIDE STAT', 'Enter new value (leave empty to revert to auto):', '', ''); if (val !== null) { await window.executeAction({action:'edit_stat', key: key, value: val}); } };
        window.editTodayEarnings = function() { window.editStat('today_rev'); };
        window.approveReview = async function(id) { await window.executeAction({action:'approve_review', id:id}); };
        window.rejectReview = async function(id) { const reason = await window.customPrompt('REVIEW REJECTION', 'Specify reason for user log:'); if(reason !== null) await window.executeAction({action:'reject_review', id:id, reason:reason}); };
        window.toggleMaintenance = async function(state) { const dur = document.getElementById('maint-duration').value; const ch = document.getElementById('maint-channel').value; if(state && !dur) return showToast('T-Minus missing', 'error'); await window.executeAction({action:'toggle_maintenance', state:state, duration:dur, channelId:ch}); };
        ...
        window.editProduct = function(id) { const p=rawStats.products[id]; if(!p) return; document.getElementById('editProdId').value=id; document.getElementById('newProdName').value=p.name; document.getElementById('newProdPrice').value=p.price; document.getElementById('newProdStock').value=p.stock||'∞'; document.getElementById('newProdLink').value=p.link; document.getElementById('newProdDesc').value=p.desc||''; document.getElementById('newProdUpsellId').value=p.upsellId||''; document.getElementById('newProdUpsellDiscount').value=p.upsellDiscount||''; document.getElementById('saveProdBtn').innerText='💾 Patch Asset'; document.getElementById('cancelEditBtn').style.display='block'; window.scrollTo({top:0, behavior:'smooth'}); };
        window.cancelEdit = function() { document.getElementById('editProdId').value=''; document.getElementById('newProdName').value=''; document.getElementById('newProdPrice').value=''; document.getElementById('newProdStock').value=''; document.getElementById('newProdLink').value=''; document.getElementById('newProdDesc').value=''; document.getElementById('newProdUpsellId').value=''; document.getElementById('newProdUpsellDiscount').value=''; document.getElementById('saveProdBtn').innerText='➕ Inject Asset'; document.getElementById('cancelEditBtn').style.display='none'; };
        window.saveProduct = async function() { const id=document.getElementById('editProdId').value; const n=document.getElementById('newProdName').value; const p=document.getElementById('newProdPrice').value; const s=document.getElementById('newProdStock').value||'∞'; const l=document.getElementById('newProdLink').value; const d=document.getElementById('newProdDesc').value; const uid=document.getElementById('newProdUpsellId').value; const udisc=document.getElementById('newProdUpsellDiscount').value; if(!n||!p) return showToast('Designation & Value required', 'error'); if(id) { await window.executeAction({action:'edit_product', id:id, name:n, price:p, stock:s, link:l, desc:d, upsellId:uid, upsellDiscount:udisc}, false); } else { await window.executeAction({action:'add_product', name:n, price:p, stock:s, link:l, desc:d, upsellId:uid, upsellDiscount:udisc}, false); } };
        window.deleteProduct = async function(id) { if(await window.customConfirm('ASSET PURGE', 'Purge asset from network?')) await window.executeAction({action:'delete_product', id:id}, false); };
        window.editBuyLink = function(id) { const l=rawStats.buy_links[id]; if(!l) return; document.getElementById('editLinkId').value=id; document.getElementById('newLinkLabel').value=l.label; document.getElementById('newLinkUrl').value=l.url; document.getElementById('saveLinkBtn').innerText='💾 Patch Gateway'; document.getElementById('cancelEditLinkBtn').style.display='block'; };
        window.cancelEditLink = function() { document.getElementById('editLinkId').value=''; document.getElementById('newLinkLabel').value=''; document.getElementById('newLinkUrl').value=''; document.getElementById('saveLinkBtn').innerText='➕ Link Gateway'; document.getElementById('cancelEditLinkBtn').style.display='none'; };
        window.saveBuyLink = async function() { const id=document.getElementById('editLinkId').value; const label=document.getElementById('newLinkLabel').value; const url=document.getElementById('newLinkUrl').value; if(!label || !url) return showToast('Label & URL required', 'error'); if(id) { await window.executeAction({action:'edit_buy_link', id:id, label:label, url:url}, false); } else { await window.executeAction({action:'add_buy_link', label:label, url:url}, false); } };
        ...
        window.sendReview = async function() { const author = document.getElementById('rev-author').value; const rating = document.getElementById('rev-rating').value; const text = document.getElementById('rev-msg').value; if(!author || !text) return showToast('Parameters missing', 'error'); await window.executeAction({ action: 'post_review', author: author, rating: rating, text: text }); document.getElementById('rev-author').value = ''; document.getElementById('rev-msg').value = ''; };
        window.loadAllMembers = async function() { document.getElementById('memberResults').innerHTML = '<p class="text-muted" style="font-family:monospace;">Syncing directory...</p>'; try { const res = await fetch('/api/members'); if (!res.ok) throw new Error('Error'); allMembersData = await res.json(); isMembersLoaded = true; window.sortMembersLocally(); } catch (e) { document.getElementById('memberResults').innerHTML = '<p class="text-pink">Network failure.</p>'; } };
        window.sortMembersLocally = function() { const sortType = document.getElementById('memberSortSelect').value; const statusFilter = document.getElementById('memberStatusSelect').value; let filtered = [...allMembersData]; if (statusFilter === 'online') { filtered = filtered.filter(m => m.status !== 'offline'); } if (sortType === 'recent') filtered.sort((a, b) => b.joinedTimestamp - a.joinedTimestamp); else if (sortType === 'oldest') filtered.sort((a, b) => a.joinedTimestamp - b.joinedTimestamp); else if (sortType === 'spent_desc') filtered.sort((a, b) => b.totalSpent - a.totalSpent); else if (sortType === 'spent_asc') filtered.sort((a, b) => a.totalSpent - b.totalSpent); else if (sortType === 'warns') filtered.sort((a, b) => b.warns.length - a.warns.length); const q = document.getElementById('memberSearchInput').value.toLowerCase(); if (q) { filtered = filtered.filter(m => m.username.toLowerCase().includes(q) || m.id.includes(q)); } renderMembers(filtered); };
        window.filterMembersLocally = window.sortMembersLocally;
        function renderMembers(members) { 
            if (members.length === 0) { document.getElementById('memberResults').innerHTML = '<p class="text-pink" style="font-family:monospace;">0 Nodes Discovered.</p>'; return; } 
            let html = ''; 
            members.forEach(function(m) { 
                let trustColor = m.isBlacklisted ? 'var(--accent-red)' : (m.totalSpent > 0 ? 'var(--accent-green)' : 'var(--accent-orange)'); 
                let trustLabel = m.isBlacklisted ? 'Blacklisted' : (m.totalSpent > 0 ? 'Verified' : 'Unverified'); 
                let safeUsername = escapeHTML(m.username); let safeNote = escapeHTML(m.note); 
                let statusIndicator = (m.status === 'online' || m.status === 'dnd' || m.status === 'idle') ? '<span style="color:#10b981; font-size:0.7em; margin-left:10px; letter-spacing:1px;">🟢 ACTIVE</span>' : '<span style="color:#94a3b8; font-size:0.7em; margin-left:10px; letter-spacing:1px;">⚪ DORMANT</span>'; 
                let ticketsHtml = ''; 
                if (m.activeTickets && m.activeTickets.length > 0) { m.activeTickets.forEach(t => { ticketsHtml += '<div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.4); padding:8px 12px; margin-top:5px; border-radius:8px; border:1px solid rgba(255,255,255,0.05);"><span style="font-family:monospace;">#' + escapeHTML(t.name) + '</span><button style="background:rgba(239, 68, 68, 0.1); border:1px solid var(--accent-red); color:var(--accent-red); border-radius:6px; cursor:pointer; padding:6px 12px;" onclick="window.modAction(\\'close_channel\\', \\'' + m.id + '\\', {channelId: \\'' + t.id + '\\'})">Close</button></div>'; }); } else ticketsHtml = '<span class="text-muted" style="font-family:monospace;">No active links</span>'; 
                let warnsHtml = ''; 
                if (m.warns && m.warns.length > 0) { m.warns.forEach((w, i) => { warnsHtml += '<div style="font-size:0.85em; color:var(--accent-orange); margin-bottom:5px; background:rgba(249,115,22,0.1); padding:5px 8px; border-radius:6px;">⚠️ Log ' + (i+1) + ': ' + escapeHTML(w.reason) + ' <span style="opacity:0.5">(' + w.date + ')</span></div>'; }); } else warnsHtml = '<span class="text-muted" style="font-family:monospace;">Clean record</span>'; 
                let historyHtml = ''; 
                if (m.history && m.history.length > 0) { m.history.forEach(h => { historyHtml += '<div style="font-size:0.85em; margin-bottom:5px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:5px;"><span style="color:var(--accent-blue);">🛒 ' + escapeHTML(h.product) + '</span> - €' + h.price + ' <br><span style="opacity:0.5; font-size:0.9em;">' + h.date + '</span></div>'; }); } else historyHtml = '<span class="text-muted" style="font-family:monospace;">No purchases</span>'; 
                ...
                html += '<div class="card" style="margin-bottom: 25px; border-left: 4px solid ' + trustColor + '; background:rgba(15,23,42,0.6);">' +
                        ...
                        '<span style="font-size:0.8rem; font-weight:600; text-transform:uppercase; letter-spacing:1px; color:var(--text-muted); display:block; margin-bottom:12px;">⚡ Execute Command:</span>' +
                        '<div style="display:flex; gap:10px; flex-wrap:wrap;">' + 
                        '<button class="admin-btn" style="margin:0; background:rgba(168,85,247,0.1); border-color:var(--accent-purple); color:var(--accent-purple);" onclick="window.editReferralCount(\\'' + m.id + '\\', ' + currentRefs + ')">🔗 Nodes (' + currentRefs + ')</button>' +
        ...
        if(typeof Chart !== 'undefined') { Chart.defaults.color = '#64748b'; Chart.defaults.font.family = 'Inter, monospace'; }
