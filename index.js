...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        ",
            "        /* SPLASH SCREEN */",
            "        .splash-screen { position: fixed; inset: 0; background: #030712; z-index: 9999; display: flex; flex-direction: column; justify-content: center; align-items: center; pointer-events: all; }",
            "        .splash-screen.hidden { display: none; }",
            "        #nexusCanvas { display: block; margin: 20px auto; width: 600px; height: 200px; }",
            "        ",
            "        /* KANBAN UI */",
            ...
            "    <div id='toast'></div>",
            "    ",
            "    <!-- NEXUS SPLASH SCREEN WITH NETWORK BAR & N FORMATION -->",
            ...
            "       <canvas id='nexusCanvas' width='600' height='200'></canvas>",
            "    </div>",
            "    <script>",
            "    (function() {",
            ...
            "        const particles = [];",
            "        // positions initiales (barre horizontale)",
            "        for (let i = 0; i < numParticles; i++) {",
            "            const x = 50 + (i / (numParticles - 1)) * 500; // de 50 à 550",
            "            const y = 100;",
            "            particles.push({ startX: x, startY: y, x: x, y: y });",
            "        }",
            "        // positions finales pour le N",
            "        const leftCount = 20;",
            "        const diagCount = 20;",
            "        const rightCount = 20;",
            "        let idx = 0;",
            "        for (let i = 0; i < leftCount; i++) {",
            "            const y = 20 + (i / (leftCount - 1)) * 160;",
            "            particles[idx].endX = 100;",
            "            particles[idx].endY = y;",
            "            idx++;",
            "        }",
            ...
            "            const x = 100 + t * 400;",
            "            const y = 20 + t * 160;",
            "            particles[idx].endX = x;",
            "            particles[idx].endY = y;",
            "            idx++;",
            "        }",
            "        for (let i = 0; i < rightCount; i++) {",
            "            const y = 20 + (i / (rightCount - 1)) * 160;",
            "            particles[idx].endX = 500;",
            "            particles[idx].endY = y;",
            "            idx++;",
            "        }",
            "        // Durées",
            "        const fillDuration = 1500; // ms",
            "        const morphDuration = 1500; // ms",
            "        const totalDuration = fillDuration + morphDuration;",
            ...
            "            const morphProgress = Math.max(0, Math.min((elapsed - fillDuration) / morphDuration, 1));",
            "",
            ...
            "                    const delay = (p.startX - 50) / 500;",
            "                    const appearProgress = Math.max(0, Math.min((fillProgress - delay) / (1 - delay), 1));",
            "                    x = p.startX;",
            "                    y = p.startY;",
