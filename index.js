...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "  let lHtml=''; if(rawStats.recent_leaves){ rawStats.recent_leaves.forEach(u=>{ let durStr='Unknown'; if(u.duration){ let d=Math.floor(u.duration/(1000*60*60*24)); let h=Math.floor((u.duration/(1000*60*60))%24); durStr=d>0?d+'d '+h+'h':h+'h'; } lHtml+='<tr><td><div style=\"display:flex; align-items:center; gap:10px;\"><img src=\"'+escapeHTML(u.avatar)+'\" style=\"width:30px; height:30px; border-radius:50%;\"/><span>'+escapeHTML(u.username)+'</span></div></td><td class=\"text-muted\">'+escapeHTML(durStr)+'</td><td class=\"text-muted\">'+escapeHTML(u.date)+'</td></tr>'; }); } document.getElementById('target-leaves').innerHTML=lHtml||'<tr><td colspan=\"3\" class=\"text-muted text-center\">No leaves recorded.</td></tr>';",
            "  let promHtml=''; if(rawStats.promo_codes){ for(const code in rawStats.promo_codes){ const info=rawStats.promo_codes[code]; const isExhausted = info.used >= info.limit; const statusColor = isExhausted ? 'var(--accent-red)' : 'var(--accent-green)'; promHtml+='<tr style=\"opacity:'+(isExhausted?'0.5':'1')+'\"><td><strong>'+escapeHTML(code)+'</strong></td><td style=\"color:'+statusColor+'; font-weight:bold;\">-'+info.discount+'%</td><td>'+info.used+' / '+info.limit+'</td><td><button class=\"admin-btn\" style=\"margin:0; padding:5px 10px; background:var(--accent-red);\" onclick=\"window.deletePromo(\\\''+encodeURIComponent(code)+'\\\')\">🗑️</button></td></tr>'; } } document.getElementById('target-promos').innerHTML=promHtml;",
