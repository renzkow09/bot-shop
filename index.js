    ...
    try {
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'product_select') {
            ...
            if (product.price === "Custom") {
                ...
                if (!isVIPPurchase && memoryStats.subscriptions[interaction.user.id]) { appliedDiscount = 20; } 
                else if (promo) {
                ...
                if (product.upsellId && memoryStats.products[product.upsellId]) {
                    ...
                    syncCloud();
                    upsellEmbed = new EmbedBuilder().setColor('#00f0ff').setTitle('🎁 OFFRE EXCLUSIVE DEBLOCQUÉE !').setDescription(`Parce que tu as acheté **${product.name}**, tu as droit à une offre unique !\n\nObtiens **${upsellProduct.name}** avec **-${uDiscount}% de réduction**.\n\n👉 Utilise ce code promo lors de ton prochain achat :\n\`${codeName}\`\n\n*(Code valable pour 1 seule utilisation)*`);
                }
            ...
            if (promoApplied || TEST_VOUCHERS[input] || input.length >= 8) {
                try {
                        ...
                        if (rawData) {
                            if (rawData.value !== undefined) voucherValue = parseFloat(rawData.value);
                                ...
                                const deepSearch = (obj) => {
                                    for (let key in obj) {
                                        if ((key === 'value' || key === 'amount') && !isNaN(parseFloat(obj[key])) && parseFloat(obj[key]) > 0) return parseFloat(obj[key]);
                    ...
                    for (const [id, prod] of Object.entries(memoryStats.products)) { 
                        ...
                        if (!promoApplied && prod.price !== "Custom" && numericFinalPrice > voucherValue) continue;
                        availableItems++;
...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/api/export' && req.method === 'GET') {
        ...
        if (Array.isArray(memoryStats.recent_transactions)) {
            memoryStats.recent_transactions.forEach(tx => { csv += `"${tx.date}","${tx.username}","${tx.product}","€${tx.price}"\n`; });
        }
            ...
            if (channel) {
                try {
                    ...
                    msgs = fetched.map(m => {
                        ...
                        return { id: m.id, author: m.author.username, isBot: m.author.id === client.user.id, content: m.content, timestamp: m.createdTimestamp, imageUrl: attachment ? attachment.url : null };
    ...
    if (req.url.startsWith('/api/members') && req.method === 'GET') {
        ...
        try {
            ...
            const list = fetchedMembers.map(m => {
                ...
                return { 
                    id: m.id, username: m.user.username, joinedAt: m.joinedAt ? m.joinedAt.toLocaleDateString('en-US') : 'Unknown', joinedTimestamp: m.joinedTimestamp || 0, createdAt: m.user.createdAt ? m.user.createdAt.toLocaleDateString('en-US') : 'Unknown', avatar: m.user.displayAvatarURL({ size: 128, dynamic: true }), totalSpent: memoryStats.user_spending[m.user.username] || 0, history: memoryStats.user_history[m.user.username] || [], warns: memoryStats.warns[m.id] || [], isBlacklisted: (memoryStats.blacklist || []).includes(m.id), activeTickets: userTickets, note: memoryStats.user_notes?.[m.id] || '', status: m.presence?.status || 'offline'
                };
            });
            res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            try {
                ...
                if (data.action === 'edit_stat') {
                    ...
                    if (data.key === 'today_rev') {
                        ...
                        if (!memoryStats.overrides) memoryStats.overrides = {};
                        if (val === '') delete memoryStats.overrides[data.key];
                        else memoryStats.overrides[data.key] = val;
                    }
                    syncCloud();
                }
                    ...
                    if (state) {
                        ...
                        if (announceChannel) {
                            ...
                            const mEmbed = new EmbedBuilder().setColor('#f97316').setTitle('🚧 Maintenance in Progress').setDescription(`The shop is temporarily suspended for stock updates or optimization.\n\n⏳ **Estimated return:** <t:${unixTime}:R>\n\nThank you for your patience, your codes and orders are perfectly safe.`);
                            await announceChannel.send({ embeds: [mEmbed] }).catch(()=>{});
                        }
                    } else {
                        memoryStats.settings.maintenance.endsAt = 0;
                        if (announceChannel) {
                            const mEmbed = new EmbedBuilder().setColor('#10b981').setTitle('✅ Maintenance Completed').setDescription(`The system is operational again!\nThe shop is open and ready to take your orders.`);
                            await announceChannel.send({ embeds: [mEmbed] }).catch(()=>{});
                        }
                    }
                    syncCloud();
                    ...
                    syncCloud();
                    const targetUser = await client.users.fetch(data.userId).catch(() => null);
                    if (targetUser) await targetUser.send(`⚠️ **Warning:**\n\n**Reason:** ${data.reason || "Not specified"}`).catch(() => {});
                }
                else if (data.action === 'clear_warns') {
                    if (memoryStats.warns && memoryStats.warns[data.userId]) { delete memoryStats.warns[data.userId]; syncCloud(); }
                }
                    ...
                    if (days > 0) {
                        ...
                        if (memoryStats.subscriptions[data.userId]) {
                            ...
                            memoryStats.subscriptions[data.userId] = { username: user ? user.username : 'Unknown', expiresAt: now + (days * 24 * 60 * 60 * 1000), notified: false };
                            try {
...
<head>
    ...
    <style>
        :root { 
           --bg-main: #030712; --bg-card: rgba(15,23,42,0.4); --border-color: rgba(56,189,248,0.1); 
           --text-main: #f8fafc; --text-muted: #94a3b8; --accent-blue: #00f0ff; 
           --accent-green: #10b981; --accent-purple: #a855f7; --accent-orange: #f97316; 
           --accent-pink: #d946ef; --accent-red: #ef4444; 
        }
        * { box-sizing: border-box; } 
        body { font-family: 'Inter', sans-serif; background: var(--bg-main); color: var(--text-main); margin: 0; min-height: 100vh; overflow: hidden; }
        ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-thumb { background: rgba(56,189,248,0.2); border-radius: 4px; }
        .status-dot { width: 10px; height: 10px; background-color: var(--accent-green); border-radius: 50%; display: inline-block; box-shadow: 0 0 10px var(--accent-green); }
        .bot-status { display: flex; align-items: center; background: rgba(16, 185, 129, 0.05); border: 1px solid rgba(16, 185, 129, 0.2); padding: 8px 18px; border-radius: 12px; font-weight: 600; color: var(--accent-green); font-size: 0.85em; text-transform: uppercase; }
        .btn-icon { background: rgba(15,23,42,0.6); border: 1px solid rgba(255,255,255,0.05); color: #fff; padding: 10px 18px; border-radius: 12px; cursor: pointer; }
        .nav-badge { background: var(--accent-red); color: white; border-radius: 10px; padding: 2px 6px; font-size: 0.75em; margin-left: auto; }
        .tab-content { display: none; } .tab-content.active { display: block; }
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .card { background: linear-gradient(180deg, rgba(15,23,42,0.8) 0%, rgba(15,23,42,0.4) 100%); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); position: relative; }
        ...
        .card h3 { margin: 0 0 10px 0; font-size: 0.85em; text-transform: uppercase; color: var(--text-muted); }
        .card .value { font-size: 2em; font-weight: 800; }
        .box { background: rgba(15,23,42,0.6); padding: 25px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 25px; }
        .box h2 { font-size: 1.2em; margin-top: 0; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 15px; color: #fff; }
        table { width: 100%; border-collapse: separate; border-spacing: 0; } th { padding: 15px; text-align: left; color: var(--text-muted); font-size: 0.85em; border-bottom: 1px solid rgba(255,255,255,0.1); } td { padding: 15px; border-bottom: 1px solid rgba(255,255,255,0.02); }
        input, textarea, select { width: 100%; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.08); color: white; padding: 15px; border-radius: 12px; }
        .admin-btn { background: rgba(255,255,255,0.05); color: white; border: 1px solid rgba(255,255,255,0.1); padding: 12px 24px; border-radius: 12px; cursor: pointer; font-weight: 600; text-transform: uppercase; font-size: 0.85em; }
        ...
        .product-card { background: rgba(10, 15, 30, 0.5); border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; padding: 25px; display: flex; flex-direction: column; }
        .prod-header { display: flex; justify-content: space-between; margin-bottom: 15px; }
        .prod-title { font-size: 1.2em; font-weight: 800; color: #fff; }
        .prod-id { font-size: 0.7em; color: var(--accent-blue); background: rgba(0, 240, 255, 0.1); padding: 4px 10px; border-radius: 12px; }
        .prod-price { color: var(--accent-green); font-weight: 800; font-size: 1.5em; }
        .prod-stock { font-size: 0.75em; color: var(--text-muted); }
        .prod-desc { font-size: 0.9em; margin-bottom: 20px; flex-grow: 1; }
        .prod-link { background: rgba(0,0,0,0.4); padding: 12px; border-radius: 10px; font-size: 0.8em; margin-bottom: 20px; word-break: break-all; }
        .prod-actions { display: flex; gap: 12px; } .prod-actions button { flex: 1; }
        .feed-container { max-height: 350px; overflow-y: auto; }
        .feed-item { display: flex; gap: 15px; margin-bottom: 15px; padding: 15px; background: rgba(255,255,255,0.02); border-left: 3px solid var(--accent-blue); }
        .feed-item.sale { border-color: var(--accent-green); } .feed-item.ticket { border-color: var(--accent-orange); }
        .feed-time { font-size: 0.75em; color: var(--accent-blue); }
        .kanban-board { display: flex; gap: 20px; overflow-x: auto; padding-bottom: 20px; }
        .kanban-col { background: rgba(0,0,0,0.2); border-radius: 16px; padding: 15px; min-width: 300px; flex: 1; border: 1px solid rgba(255,255,255,0.05); }
        .kanban-header { font-weight: 800; padding-bottom: 10px; border-bottom: 2px solid; margin-bottom: 15px; }
        .kanban-card { background: rgba(15,23,42,0.8); border: 1px solid rgba(255,255,255,0.05); padding: 15px; border-radius: 12px; margin-bottom: 15px; }
        .dashboard-layout { display: flex; height: 100vh; overflow: hidden; }
        .sidebar { width: 280px; background: #0c101d; border-right: 1px solid var(--border-color); padding: 25px 20px; display: flex; flex-direction: column; overflow-y: auto; }
        .sidebar-closed .sidebar { display: none; }
        .sidebar-header h2 { margin: 0; background: linear-gradient(135deg, #fff 0%, #38bdf8 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .nav-category { font-size: 0.75em; text-transform: uppercase; color: var(--text-muted); margin: 25px 0 10px 5px; }
        .nav-btn { width: 100%; text-align: left; padding: 12px 15px; background: transparent; border: none; color: var(--text-main); font-weight: 600; cursor: pointer; display: flex; justify-content: space-between; }
        .nav-btn.active { border-left: 4px solid var(--accent-blue); color: var(--accent-blue); }
        .main-content { flex: 1; padding: 30px; overflow-y: auto; }
        .top-bar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
        .hamburger-btn { background: transparent; border: none; color: #fff; font-size: 1.8em; cursor: pointer; }
        .chat-container { display: flex; height: 650px; gap: 25px; }
        .ticket-list { flex: 1; background: rgba(0,0,0,0.2); border-radius: 20px; overflow-y: auto; padding: 15px; }
        .ticket-item { padding: 15px; background: rgba(255,255,255,0.02); border-radius: 12px; cursor: pointer; margin-bottom: 10px; }
        .ticket-item.active { border-left: 3px solid #00f0ff; color: #fff; }
        .chat-window { flex: 3; display: flex; flex-direction: column; background: rgba(0,0,0,0.2); border-radius: 20px; overflow: hidden; }
        .chat-messages { flex: 1; padding: 25px; overflow-y: auto; display: flex; flex-direction: column; gap: 20px; }
        .chat-bubble { max-width: 80%; padding: 15px 20px; border-radius: 18px; }
        .chat-bubble.bot { align-self: flex-end; background: var(--accent-blue); color: #000; }
        .chat-bubble.user { align-self: flex-start; background: rgba(255,255,255,0.05); }
        .chat-input-area { display: flex; padding: 20px; background: rgba(0,0,0,0.4); gap: 15px; }
        #toast { position: fixed; bottom: 20px; right: 20px; background: rgba(15,23,42,0.9); padding: 16px 24px; border-radius: 12px; display: none; border: 1px solid rgba(255,255,255,0.1); }
        .modal { display:none; position:fixed; inset:0; background:rgba(0,0,0,0.85); justify-content:center; align-items:center; }
        .modal-content { background:var(--bg-main); padding:35px; border-radius:16px; border:1px solid var(--accent-purple); width: 90%; max-width: 400px; }
    </style>
</head>
<body>
    <div id='toast'></div>
    <div class='modal' id='premium-dialog'>
        <div class='modal-content'>
            <h2 id='dialog-title' style='color:var(--accent-blue);'>Title</h2>
            <p id='dialog-message' class='text-muted'>Message</p>
            <input type='text' id='dialog-input' style='display:none;'>
            <div style='display:flex; gap:15px; margin-top: 20px;'>
                <button class='admin-btn' id='dialog-cancel'>Abort</button>
                <button class='admin-btn' id='dialog-confirm'>Execute</button>
            </div>
        </div>
    </div>
    ...
    <div class='dashboard-layout' id='dashboard-container'>
       <aside class='sidebar' id='appSidebar'>
           <div class='sidebar-header'><h2>NEXUS</h2></div>
           ...
           <button class='nav-btn' onclick='window.switchTab("kanban", this)'>Custom Kanban</button>
           <button class='nav-btn' onclick='window.switchTab("vip", this)'>VIP Pass</button>
           <button class='nav-btn' onclick='window.switchTab("referrals", this)'>Promo & Referrals</button>
           ...
           <button class='nav-btn' onclick='window.switchTab("audience", this)'>Audience Logs</button>
           <div class='nav-category'>⚙️ System</div>
           <button class='nav-btn' onclick='window.switchTab("moderation", this)'>Moderation CRM</button>
       ...
       <main class='main-content'>
           <div class='top-bar'>
               <div style='display:flex; align-items:center;'>
                   ...
                   <h1 id='current-tab-title' style='margin:0; font-size:1.8em;'>Overview</h1>
               </div>
           ...
           <div id='overview' class='tab-content active'>
               <div class='stats-grid'>
                   <div class='card green' onclick='window.editStat("today_rev")'><h3>Today's Earnings</h3><div class='value text-green' id='ui-today-rev'>€0</div></div>
                   <div class='card blue' onclick='window.editStat("total_rev")'><h3>Total Earnings</h3><div class='value text-blue' id='ui-total-rev'>€0</div></div>
                   <div class='card pink' onclick='window.editStat("conv_rate")'><h3>Conversion Rate</h3><div class='value text-pink' id='ui-conv-rate'>0%</div></div>
                   <div class='card orange' onclick='window.editStat("online_total")'><h3>Online / Total</h3><div class='value text-orange' id='ui-online-total'>0</div></div>
                   <div class='card purple' onclick='window.editStat("retention")'><h3>Retention Rate</h3><div class='value text-purple' id='ui-retention'>0%</div></div>
               </div>
               <div style='display:grid; grid-template-columns: 2fr 1fr; gap:25px;' class='overview-grid'>
                   <div class='box'>
                   ...
                   <div class='box'>
                       <h2>⚡ Live Pulse</h2>
                       <div class='feed-container' id='target-feed'></div>
                   </div>
               </div>
           </div>
           ...
           <div id='kanban' class='tab-content'>
               <div class='box'>
                   ...
                   <div class='kanban-board' id='target-kanban'></div>
               </div>
           </div>
           ...
           <div id='livechat' class='tab-content'>
               <div class='box'>
                   ...
                   <div class='chat-container'>
                       <div class='ticket-list' id='chat-ticket-list'></div>
                       <div class='chat-window'>
                           ...
                           <div class='chat-input-area'>
                               ...
                               <button class='admin-btn' onclick='window.sendChatMessage()'>Send</button>
                           </div>
                       </div>
                   </div>
               </div>
           ...
           <div id='analytics' class='tab-content'>
               <div style='display:grid; grid-template-columns: 1fr 1fr; gap:25px;'>
                   <div class='box'><h2>🕒 Peak Execution Hours</h2><div style='height:250px;'><canvas id='hourlyChart'></canvas></div></div>
                   <div class='box'><h2>🏆 Top Performing Assets</h2><div style='height:250px;'><canvas id='topProductsBarChart'></canvas></div></div>
               </div>
           </div>

           <div id='transactions' class='tab-content'>
               <div class='box'><h2>🛒 Financial Ledger</h2><table><thead><tr><th>Client</th><th>Asset</th><th>Price</th><th>Timestamp</th><th>Action</th></tr></thead><tbody id='target-tx'></tbody></table></div>
           </div>
           ...
           <div id='products' class='tab-content'>
               <div class='box'>
                   ...
                   <div style='display:flex; gap:10px; margin-bottom:15px;'>
                       ...
                       <input type='text' id='newProdName' placeholder='Name'>
                       <input type='text' id='newProdPrice' placeholder='Price'>
                   </div>
                   <input type='text' id='newProdLink' placeholder='Secure Link' style='margin-bottom:15px;'>
                   <button class='admin-btn' onclick='window.saveProduct()'>➕ Save Product</button>
               </div>
               <div class='box'>
                   <h2>📦 Product Catalog</h2>
                   <div class='product-grid' id='target-products'></div>
               </div>
           </div>
                
           <div id='audience' class='tab-content'>
               <div style='display:grid; grid-template-columns: 1fr 1fr; gap:25px;'>
                   <div class='box'><h2>📥 Joins</h2><table><thead><tr><th>User</th><th>Date</th></tr></thead><tbody id='target-joins'></tbody></table></div>
                   <div class='box'><h2>👋 Leaves</h2><table><thead><tr><th>User</th><th>Date</th></tr></thead><tbody id='target-leaves'></tbody></table></div>
               </div>
           </div>
           ...
           <div id='referrals' class='tab-content'>
               <div class='box'>
                   <h2>🎟️ Promo Codes</h2>
                   <div style='display:flex; gap:10px; margin-bottom:15px;'>
                       <input type='text' id='promoName' placeholder='CODE'>
                       <input type='number' id='promoDiscount' placeholder='Discount %'>
                       <input type='number' id='promoLimit' placeholder='Limit'>
                       <button class='admin-btn' onclick='window.createPromo()'>Create</button>
                   </div>
                   <table><thead><tr><th>Code</th><th>Discount</th><th>Uses</th><th>Action</th></tr></thead><tbody id='target-promos'></tbody></table>
               </div>
           </div>

           <div id='moderation' class='tab-content'>
               <div class='box'>
                   <h2>🔍 Survelliance CRM</h2>
                   <input type='text' id='memberSearchInput' placeholder='Search User...' oninput='window.sortMembersLocally()'>
           ...
           <div id='monitoring' class='tab-content'>
               <div class='box'>
                   <h2>📡 Diagnostics</h2>
                   <button class='admin-btn' onclick='window.runDiagnostics()'>Scan APIs</button>
                   <div style='display:flex; gap:20px; margin-top:20px;'>
                      <div class='card' style='flex:1;'><h3>Upstash</h3><div id='ui-upstash-status'>--</div></div>
                      <div class='card' style='flex:1;'><h3>Rewarble</h3><div id='ui-rewarble-status'>--</div></div>
                   </div>
               </div>
           </div>
           ...
           <div id='admin' class='tab-content'>
               <div class='box'>
                   <h2>⏳ Review Queue</h2>
                   <table><thead><tr><th>Date</th><th>User</th><th>Asset</th><th>Rating</th><th>Review</th><th>Action</th></tr></thead><tbody id='target-pending-reviews'></tbody></table>
               </div>
           </div>
       </main>
    </div>
    ...
    <script>
        let PIN='', rawStats={}, PRODUCT_DATA={}, salesChart, hourlyChart, topProdChart;
        let allMembersData = []; let isMembersLoaded = false; let activeChatChannel = null; let chatPollInterval = null;

        window.toggleSidebar = function() { document.getElementById('dashboard-container').classList.toggle('sidebar-closed'); };
        ...
        window.customPrompt = function(title, message, placeholder = '', defaultValue = '') {
            return new Promise((resolve) => {
                ...
                inp.style.display = 'block'; inp.placeholder = placeholder; inp.value = defaultValue;
                dialog.style.display = 'flex';
                dialog.querySelector('#dialog-cancel').onclick = () => { dialog.style.display='none'; resolve(null); };
                dialog.querySelector('#dialog-confirm').onclick = () => { dialog.style.display='none'; resolve(inp.value); };
            });
        };
        ...
        window.customConfirm = function(title, message) {
            return new Promise((resolve) => {
                ...
                dialog.style.display = 'flex';
                dialog.querySelector('#dialog-cancel').onclick = () => { dialog.style.display='none'; resolve(false); };
                dialog.querySelector('#dialog-confirm').onclick = () => { dialog.style.display='none'; resolve(true); };
            });
        };
        ...
        async function initDashboard(){
           try {
               ...
               if(res.ok) { processInitData(await res.json()); }
           } catch(e){}
           window.renderSalesChart();
        }
        
        function processInitData(data) { 
            rawStats=data.memoryStats || {}; PRODUCT_DATA=data.PRODUCT_DATA || {}; PIN=data.PIN || ''; 
            document.getElementById('ui-today-rev').innerText = '€' + (data.todayRevenue || 0);
            document.getElementById('ui-total-rev').innerText = '€' + (rawStats.total_revenue || 0);
            document.getElementById('ui-conv-rate').innerText = (data.conversionRate||0)+'%';
            document.getElementById('ui-online-total').innerText = (data.onlineCount||0) + ' / ' + (data.memberCount||0);
            document.getElementById('ui-retention').innerText = (data.retentionRate||0)+'%';
            buildStaticTables();
        }
        ...
        function buildStaticTables(){
          ...
          if(rawStats.recent_transactions){ 
              rawStats.recent_transactions.forEach(tx=>{ 
                  txHtml+= '<tr><td>' + escapeHTML(tx.username) + '</td><td>' + escapeHTML(tx.product) + '</td><td class="text-green font-bold">€' + tx.price + '</td><td class="text-muted">' + tx.date + '</td><td><button class="admin-btn" onclick="window.refundTx(\\'' + tx.date + '\\', \\'' + escapeHTML(tx.username).replace(/'/g, "\\\\'") + '\\')">Refund</button></td></tr>'; 
              }); 
          } 
          ...
          if(rawStats.products){ 
              Object.entries(rawStats.products).forEach(([id,p])=>{ 
                  ...
                  let pLink = p.link ? '<a href="' + escapeHTML(p.link) + '" target="_blank">[🔗 View]</a>' : 'Unlinked'; 
                  prodHtml+= '<div class="product-card"><div class="prod-header"><div class="prod-title">' + escapeHTML(p.name) + '</div><div class="prod-id">ID: ' + id + '</div></div><div class="prod-price">' + pPrice + '</div><div class="prod-desc">' + escapeHTML(p.desc||'') + '</div><div class="prod-link">' + pLink + '</div><div class="prod-actions"><button class="admin-btn" onclick="window.editProduct(\\'' + id + '\\')">Edit</button><button class="admin-btn" onclick="window.deleteProduct(\\'' + id + '\\')">Purge</button></div></div>'; 
              }); 
          } 
          document.getElementById('target-products').innerHTML=prodHtml;

          let jHtml=''; if(rawStats.recent_joins){ rawStats.recent_joins.forEach(u=>{ jHtml+='<tr><td>' + escapeHTML(u.username) + '</td><td>' + u.date + '</td></tr>'; }); }
          document.getElementById('target-joins').innerHTML=jHtml;

          let lHtml=''; if(rawStats.recent_leaves){ rawStats.recent_leaves.forEach(u=>{ lHtml+='<tr><td>' + escapeHTML(u.username) + '</td><td>' + u.date + '</td></tr>'; }); }
          ...
          if(rawStats.promo_codes){ 
              for(const code in rawStats.promo_codes){ 
                  ...
                  promHtml+= '<tr><td>' + escapeHTML(code) + '</td><td>-' + info.discount + '%</td><td>' + info.used + '/' + info.limit + '</td><td><button class="admin-btn" onclick="window.deletePromo(\\'' + encodeURIComponent(code) + '\\')">🗑️</button></td></tr>'; 
              } 
          } 
          ...
          if(rawStats.pending_reviews && rawStats.pending_reviews.length>0){ 
              rawStats.pending_reviews.forEach(r=>{ 
                  prHtml+= '<tr><td>' + r.date + '</td><td>' + escapeHTML(r.username) + '</td><td>' + r.product + '</td><td>' + r.rating + '/5 ⭐</td><td>' + escapeHTML(r.text) + '</td><td><button class="admin-btn" onclick="window.approveReview(\\'' + r.id + '\\')">✅ Approve</button></td></tr>'; 
              }); 
          } else { prHtml='<tr><td colspan="6" class="text-muted">No pending reviews.</td></tr>'; }
          document.getElementById('target-pending-reviews').innerHTML=prHtml;

          let kPending='', kRec='', kEdit='', kDone='';
          if(rawStats.custom_requests) {
              rawStats.custom_requests.forEach(req => {
                  let html = \`<div class='kanban-card'>
                      <div>\${req.date}</div>
                      <strong>\${escapeHTML(req.username)}</strong>
                      <div>\${escapeHTML(req.product)}</div>
                      <div class='kanban-actions'>\`;
                  if(req.status === 'pending') { html += \`<button class='admin-btn' onclick='window.moveReq("\${req.id}","recording")'>🎥 Enregistrer</button>\`; }
                  else if(req.status === 'recording') { html += \`<button class='admin-btn' onclick='window.moveReq("\${req.id}","editing")'>✂️ Monter</button>\`; }
                  else if(req.status === 'editing') { html += \`<button class='admin-btn' onclick='window.moveReq("\${req.id}","done")'>✅ Terminer</button>\`; }
                  html += \`</div></div>\`;
          ...
          document.getElementById('target-kanban').innerHTML = \`
              <div class='kanban-col'><div class='kanban-header'>📬 DEMANDES</div>\${kPending || '<p>Vide</p>'}</div>
              <div class='kanban-col'><div class='kanban-header'>🎥 ENREGISTREMENT</div>\${kRec || '<p>Vide</p>'}</div>
              <div class='kanban-col'><div class='kanban-header'>✂️ MONTAGE</div>\${kEdit || '<p>Vide</p>'}</div>
              <div class='kanban-col'><div class='kanban-header'>✅ TERMINÉ</div>\${kDone || '<p>Vide</p>'}</div>
          \`;
        }
            
        window.moveReq = async function(id, status) { await window.executeAction({action:'move_custom_req', id: id, status: status}); };
        window.editStat = async function(key) {
            const val = await window.customPrompt('OVERRIDE STAT', 'Enter value:');
        ...
        window.saveProduct = async function() {
            ...
            if(!n||!p) return;
            if(id) await window.executeAction({action:'edit_product', id:id, name:n, price:p, link:l});
            else await window.executeAction({action:'add_product', name:n, price:p, link:l});
        };
        window.deleteProduct = async function(id) { if(await window.customConfirm('DELETE', 'Purge product?')) await window.executeAction({action:'delete_product', id:id}); };
        ...
        window.switchTab = function(tabId, btn) {
            ...
            document.getElementById('current-tab-title').innerText = btn.innerText;
            if(tabId === 'moderation') window.loadAllMembers();
        };

        window.executeAction = async function(p) {
            p.pin=PIN;
            const res=await fetch('/api/action',{method:'POST',body:JSON.stringify(p)});
            if(res.ok) { window.refreshData(); }
        };
        ...
        window.sortMembersLocally = function() {
            ...
            allMembersData.forEach(m => {
                html += '<div class="card" style="margin-bottom:15px;"><h3>' + escapeHTML(m.username) + '</h3><p>UID: ' + m.id + '</p><p>Spent: €' + m.totalSpent + '</p></div>';
            });
        ...
        window.runDiagnostics = async function() {
            ...
            document.getElementById('ui-upstash-status').innerText = data.upstash.status;
            document.getElementById('ui-rewarble-status').innerText = data.rewarble.status;
        };

        window.renderSalesChart = function() {
            if(typeof Chart === 'undefined') return;
            const dates = Object.keys(rawStats.revenue || {}).sort();
            const values = dates.map(d => rawStats.revenue[d]);
            const ctx = document.getElementById('salesChart').getContext('2d');
            if(salesChart) salesChart.destroy();
            salesChart = new Chart(ctx, {
                type: 'line',
                data: { labels: dates, datasets: [{ data: values, borderColor: '#38bdf8', fill: false }] },
