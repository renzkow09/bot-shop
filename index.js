...
client.on('messageCreate', async (message) => {
    try {
        ...
        if (message.channel?.name?.startsWith('shop-')) {
            ...
            // 💳 VALEUR DU BON EXTRAITE (SI C'EST UN BON REWARBLE)
            let voucherValue = 0; 
            ...
            if (promoApplied || TEST_VOUCHERS[input] || input.length >= 8) {
                try {
                    if (!promoApplied && !TEST_VOUCHERS[input]) {
                        ...
                        // 🔥 NOUVEAU: Extraction de la valeur du bon depuis l'API Rewarble
                        // Si l'API renvoie la valeur dans `apiResponse.data.value`, on la stocke.
                        // Adapte "apiResponse.data.value" selon la documentation exacte de Rewarble.
                        if (apiResponse.data && apiResponse.data.value) {
                            voucherValue = parseFloat(apiResponse.data.value);
                        } else {
                            // Valeur par défaut si l'API ne retourne pas le montant
                            voucherValue = 100; 
                        }
                    } else if (TEST_VOUCHERS[input]) {
                        voucherValue = TEST_VOUCHERS[input]; // Valeur du bon de test
                    }
                    ...
                    for (const [id, prod] of Object.entries(memoryStats.products)) { 
                        ...
                        // 🔥 NOUVEAU: VERIFICATION DE LA SOMME 🔥
                        // On n'ajoute pas l'article au menu si le client n'a pas mis un bon avec une valeur suffisante
                        // On ignore la vérification pour les "Custom" ou si un promo code est utilisé
                        if (!promoApplied && prod.price !== "Custom" && numericFinalPrice > voucherValue) {
                            continue; // Le produit est trop cher pour le code du client
                        }
                    ...
                    if (menu.options.length === 0) {
                        return message.reply(`❌ **Insufficient Funds.** The code you provided (€${voucherValue}) is not enough to buy any available items.`);
                    }

                    // 🔥 NOUVEAU: ANNONCE DE LA VALEUR AU CLIENT
                    let replyMsg = `✅ **Code validated! Value: €${voucherValue}. Select your item below:**`;
                    if (promoApplied) replyMsg = `✅ **Promo Code Accepted (-${promoApplied.discount}%)! Select your item below:**`;
                    else if (isUserVIP) replyMsg = `👑 **VIP Status Active! (-20% on all items). Value: €${voucherValue}. Select your item below:**`;
...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        };",
            "        ",
            "        window.deleteProduct = async function(id) { if(await window.customConfirm('ASSET PURGE', 'Purge asset from network?')) await window.executeAction({action:'delete_product', id:id}, true); };",
            "        ",
            ...
            "        window.deleteBuyLink = async function(id) { if(await window.customConfirm('GATEWAY SEVER', 'Sever this gateway link?')) await window.executeAction({action:'delete_buy_link', id:id}, true); };",
            "",
