...
<body>
    ...
    <script>
        ...
        window.setTheme = function(color) {
            const themes = {
                green: { hex: getComputedStyle(document.documentElement).getPropertyValue("--accent-green"), rgb: '16, 185, 129' },
            ...
            if(window.salesChart) window.updateSalesChart(window.lastChartDays || 7);
        };
...
if(savedTheme) window.setTheme(savedTheme);
        ...
        function updateMaintenanceBadge(m) { 
            ...
            if(m && m.active && Date.now() < m.endsAt) { 
                ...
                botStatus.style.background = 'rgba(' + getComputedStyle(document.documentElement).getPropertyValue("--accent-green-rgb").trim() + ', 0.1)'; botStatus.style.borderColor = 'rgba(' + getComputedStyle(document.documentElement).getPropertyValue("--accent-green-rgb").trim() + ', 0.2)'; 
            } 
        }
        ...
        function buildStaticTables(){
          ...
          if(rawStats.promo_codes){ 
              for(const code in rawStats.promo_codes){ 
                  ...
                  const statusColor = isExhausted ? 'var(--accent-red)' : getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(); 
          ...
          if(rawStats.custom_requests && rawStats.custom_requests.length>0) {
              rawStats.custom_requests.forEach(req => {
                  ...
                  if(req.status === 'pending') { html += \`<button class='admin-btn' style='color:var(--accent-orange)' onclick='window.moveReq("\${req.id}","recording")'>🎥 Enregistrer</button>\`; }
                  else if(req.status === 'recording') { html += \`<button class='admin-btn' style='color:var(--accent-purple)' onclick='window.moveReq("\${req.id}","editing")'>✂️ Monter</button>\`; }
                  else if(req.status === 'editing') { html += \`<button class='admin-btn' style='color:var(--accent-green)' onclick='window.moveReq("\${req.id}","done")'>✅ Terminer</button>\`; }
                  html += \`</div></div>\`;
        ...
        function renderMembers(members) { 
            ...
            members.forEach(function(m) { 
                let trustColor = m.isBlacklisted ? 'var(--accent-red)' : (m.totalSpent > 0 ? getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim() : 'var(--accent-orange)'); 
                ...
                html += '<div class="card" style="margin-bottom: 25px; border-left: 4px solid ' + trustColor + ';">' +
                            ...
                            '<div style="margin-bottom:20px; padding-top:20px; border-top:0.5px solid rgba(255,255,255,0.05);">' +
                                ...
                                '<textarea id="note-' + m.id + '" placeholder="Inject private parameters..." style="min-height:80px;" onblur="window.saveUserNote(\\'' + m.id + '\\')">' + safeNote + '</textarea>' +
                            '</div>' +
                ...
                html += '<button class="admin-btn" style="margin:0;" onclick="window.editReferralCount(\\'' + m.id + '\\', ' + currentRefs + ')">🔗 Nodes (' + currentRefs + ')</button>' +
                        '<button class="admin-btn" style="margin:0;" onclick="window.openDirectContact(\\'' + m.id + '\\')">💬 Inject DM</button>' +
                        '<button class="admin-btn" style="margin:0;" onclick="window.modAction(\\'mute\\', \\'' + m.id + '\\', {duration: 15})">🔇 15m</button>' +
                        '<button class="admin-btn" style="margin:0;" onclick="window.modAction(\\'mute\\', \\'' + m.id + '\\', {duration: 60})">🔇 1h</button>' +
                        '<button class="admin-btn" style="margin:0; color:var(--accent-red);" onclick="window.modAction(\\'mute\\', \\'' + m.id + '\\', {duration: 1440})">🔇 1d</button>' +
                        '<button class="admin-btn" style="margin:0; color:var(--accent-red);" onclick="window.modAction(\\'mute\\', \\'' + m.id + '\\', {duration: 10080})">🔇 1w</button>' +
                        '<button class="admin-btn" style="margin:0; color:var(--accent-orange);" onclick="window.modAction(\\'warn\\', \\'' + m.id + '\\')">⚠️ Warn</button>' +
                        '<button class="admin-btn" style="margin:0;" onclick="window.modAction(\\'clear_warns\\', \\'' + m.id + '\\')">🧹 Clear Log</button>' +
                        '<button class="admin-btn" style="margin:0; background:rgba(239,68,68,0.2); color:#fff; border-color:transparent;" onclick="window.modAction(\\'kick\\', \\'' + m.id + '\\')">👢 Kick</button>' +
                        '<button class="admin-btn" style="margin:0; background:var(--accent-red); color:#fff; border-color:transparent;" onclick="window.modAction(\\'ban\\', \\'' + m.id + '\\')">🔨 Ban</button>' +
                        '<button class="admin-btn" style="width:auto; margin:0; background:rgba(0,0,0,0.5); color:' + (m.isBlacklisted ? getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim() : 'var(--accent-red)') + ';" onclick="window.modAction(\\'toggle_blacklist\\', \\'' + m.id + '\\')">' + (m.isBlacklisted ? '✅ Restore Access' : '🚫 Sever Access') + '</button>' +
                        '</div></div></div>'; 
            }); 
        ...
        window.renderSalesChart = function(days) { if(typeof Chart === 'undefined') return; let dates = Object.keys(rawStats.revenue || {}).sort(); let values = dates.map(d => rawStats.revenue[d]); if (days > 0 && dates.length > days) { dates = dates.slice(-days); values = values.slice(-days); } const ctxSales = document.getElementById('salesChart').getContext('2d'); let grad = ctxSales.createLinearGradient(0,0,0,400); grad.addColorStop(0, 'rgba(' + getComputedStyle(document.documentElement).getPropertyValue("--accent-green-rgb").trim() + ', 0.4)'); grad.addColorStop(1, 'transparent'); if(salesChart) salesChart.destroy(); salesChart = new Chart(ctxSales, { type: 'line', data: { labels: dates.length?dates:['No Data'], datasets: [{ data: values.length?values:[0], borderColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(), backgroundColor: grad, fill: true, tension: 0.4, pointHoverBackgroundColor: '#fff', pointHoverBorderColor: 'rgba(' + getComputedStyle(document.documentElement).getPropertyValue("--accent-green-rgb").trim() + ', 1)', pointHoverBorderWidth: 4, pointRadius: 0, pointHitRadius: 20 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { duration: 1500, easing: 'easeOutQuart' }, plugins: { legend: { display: false } }, interaction: { intersect: false, mode: 'index' }, scales: { x: { display: false }, y: { grid: { color: 'rgba(255,255,255,0.05)'}, border: { dash: [4, 4], display: false } } } } }); };
        ...
        function renderAnalyticsCharts() { 
           ...
           const ctxHourly = document.getElementById('hourlyChart').getContext('2d'); if(hourlyChart) hourlyChart.destroy(); hourlyChart = new Chart(ctxHourly, { type: 'bar', data: { labels: Array.from({length: 24}, (_, i) => i+'h'), datasets: [{ label: 'Sales', data: rawStats.analytics.hourly_sales || Array(24).fill(0), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(), hoverBackgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green-hover").trim(), borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { duration: 1000, easing: 'easeOutQuart' }, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' }, border: {display: false} }, x: { grid: { display: false }, border: {display: false} } } } });
           const prodIds = Object.keys(rawStats.product_sales || {}); const prodLabels = prodIds.map(id => rawStats.products[id] ? rawStats.products[id].name : 'Unknown'); const prodData = Object.values(rawStats.product_sales || {}); const ctxTopProd = document.getElementById('topProductsBarChart').getContext('2d'); if(topProdChart) topProdChart.destroy(); topProdChart = new Chart(ctxTopProd, { type: 'bar', data: { labels: prodLabels.length?prodLabels:['No Data'], datasets: [{ label: 'Sales', data: prodData.length?prodData:[0], backgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(), hoverBackgroundColor: '#fff', borderRadius: 6 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, animation: { duration: 1000, easing: 'easeOutQuart' }, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.05)' }, border: {display: false} }, y: { grid: { display: false }, border: {display: false} } } } });
           const catRevs = {}; Object.entries(rawStats.product_sales || {}).forEach(([id, count]) => { const p = rawStats.products[id]; if(p && p.price !== 'Custom'){ const cat = p.category || 'Other'; if(!catRevs[cat]) catRevs[cat] = 0; catRevs[cat] += (parseInt(p.price) * count); } }); const ctxCat = document.getElementById('categoryRevenueChart').getContext('2d'); if(catChart) catChart.destroy(); catChart = new Chart(ctxCat, { type: 'polarArea', data: { labels: Object.keys(catRevs).length?Object.keys(catRevs):['No Data'], datasets: [{ data: Object.values(catRevs).length?Object.values(catRevs):[0], backgroundColor: [getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(), getComputedStyle(document.documentElement).getPropertyValue("--accent-green-hover").trim(), '#059669', '#f59e0b', '#ef4444'], hoverBackgroundColor: ['#fff', '#fff', '#fff', '#fff', '#fff'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, animation: { animateScale: true, animateRotate: true, duration: 1000, easing: 'easeOutQuart' }, plugins: { legend: { position: 'right', labels: {color: '#8e8e93', font: { family: '-apple-system' }} } } } });
           
           const dowSales = { 'Sun':0, 'Mon':0, 'Tue':0, 'Wed':0, 'Thu':0, 'Fri':0, 'Sat':0 }; const daysArr = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']; Object.entries(rawStats.revenue || {}).forEach(([dateStr, val]) => { const d = new Date(dateStr); if(!isNaN(d)) { dowSales[daysArr[d.getDay()]] += parseFloat(val); } }); const ctxDow = document.getElementById('dowChart').getContext('2d'); if(window.dowChartInst) window.dowChartInst.destroy(); window.dowChartInst = new Chart(ctxDow, { type: 'bar', data: { labels: daysArr, datasets: [{ label: 'Revenue (£)', data: daysArr.map(d=>dowSales[d]), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green").trim(), hoverBackgroundColor: getComputedStyle(document.documentElement).getPropertyValue("--accent-green-hover").trim(), borderRadius: 8 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { grid: { color: 'rgba(255,255,255,0.05)' }, border: {display: false} }, x: { grid: { display: false }, border: {display: false} } } } });
           const ticketsOpened = rawStats.analytics?.tickets_opened || 0; const salesClosed = rawStats.total_transactions || 0; const ctxFunnel = document.getElementById('funnelChart').getContext('2d'); if(window.funnelChartInst) window.funnelChartInst.destroy(); window.funnelChartInst = new Chart(ctxFunnel, { type: 'doughnut', data: { labels: ['Tickets Opened (No Purchase)', 'Successful Sales'], datasets: [{ data: [Math.max(0, ticketsOpened - salesClosed), salesClosed], backgroundColor: ['rgba(239, 68, 68, 0.8)', 'rgba(' + getComputedStyle(document.documentElement).getPropertyValue("--accent-green-rgb").trim() + ', 0.8)'], hoverOffset: 4, borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins: { legend: { position: 'bottom', labels: { color: '#8e8e93' } } } } });
        }
