...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/api/action' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                if (data.action === 'edit_referral_count') {
                    if (!memoryStats.referrals) memoryStats.referrals = {};
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            ".status-dot { width: 12px; height: 12px; background-color: var(--accent-green); border-radius: 50%; display: inline-block; animation: pulseGreen 2s infinite; margin-right: 8px; }",
            ".bot-status { display: flex; align-items: center; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.2); padding: 8px 15px; border-radius: 8px; font-weight: bold; color: var(--accent-green); font-size: 0.9em; }",
            ...
            // 🌟 NOUVELLE ANIMATION DE CHARGEMENT: "Réseau vers N"
            ...
            // 🌟 NOUVEAU: Rafraîchissement invisible des données sans recharger la page
            "window.refreshDataSilently = async function() { try{ const res=await fetch('/api/init-data'); if(res.ok){ const data=await res.json(); rawStats=data.memoryStats; document.getElementById('ui-today-rev').innerText='€'+data.todayRevenue; document.getElementById('ui-total-rev').innerText='€'+(rawStats.total_revenue||0); document.getElementById('ui-conv-rate').innerText=data.conversionRate+'%'; buildStaticTables(); if(isMembersLoaded) window.loadAllMembers(); try { window.cancelEdit(); window.cancelEditLink(); document.getElementById('promoName').value=''; document.getElementById('promoDiscount').value=''; document.getElementById('promoLimit').value=''; } catch(e) {} } }catch(e){} };",
            
            // 🌟 NOUVEAU: executeAction utilise maintenant la fonction silencieuse
