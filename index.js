        ...
        if (message.channel?.name?.startsWith('shop-')) {
            ...
            if (promoApplied || TEST_VOUCHERS[input] || input.length >= 8) {
                try {
                    ...
                    // FIX: Remplacement de TEST_VOUVHERS par TEST_VOUCHERS pour éliminer l'erreur fatale au parsing
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; } 
        ...
        .sidebar { width: 280px; background: #0c101d; border-right: 1px solid var(--border-color); padding: 25px 20px; display: flex; flex-direction: column; overflow-y: auto; z-index: 10000; flex-shrink: 0; transition: transform 0.25s cubic-bezier(0.25, 1, 0.5, 1), margin-right 0.25s cubic-bezier(0.25, 1, 0.5, 1); -webkit-font-smoothing: antialiased; transform: translate3d(0, 0, 0); backface-visibility: hidden; }
        ...
        .hamburger-btn { block; background: transparent; border: none; color: #fff; font-size: 1.8em; cursor: pointer; padding: 0; margin-right: 15px; transition: transform 0.2s; z-index: 10001; }
        ...
        .modal-content { background:var(--bg-main); padding:35px; border-radius:16px; border:1px solid var(--accent-purple); text-align:center; max-width:400px; box-shadow: 0 10px 50px rgba(168,85,247,0.3); animation: zoomIn 0.3s forwards;} @media screen and (max-width: 900px){.sidebar{position: fixed; top: 0; left: 0; height: 100vh; transform: translate3d(-100%, 0, 0); width: 280px; box-shadow: 20px 0 50px rgba(0,0,0,0.5);}.sidebar.open{transform: translate3d(0, 0, 0)}.sidebar-closed .sidebar {transform: translate3d(-100%, 0, 0) !important; margin-right: 0 !important}.dashboard-layout{flex-direction: column;} .main-content{padding: 15px;}.overview-grid,.chat-container{grid-template-columns: 1fr !important; flex-direction: column; height: auto;} .ticket-list{height: 200px;}}
    </style>
</head>
<body>
    <div id='toast'></div>
...
http.createServer(async (req, res) => {
    try {
        ...
        if (req.url === '/api/action' && req.method === 'POST') {
            ...
            req.on('end', async () => {
                try {
                    const data = JSON.parse(body);
                    const guild = client.guilds.cache.first(); if (!guild) { res.writeHead(404); return res.end('Guild not found'); }
                        ...
                        if (Array.isArray(memoryStats.recent_transactions)) {
                            ...
                            if (txIndex > -1) {
                                ...
                                if (Array.isArray(memoryStats.user_history[tx.username])) {
                                    ...
                                    if (histIdx > -1) memoryStats.user_history[tx.username].splice(histIdx, 1);
                                }
    ...
    } catch (globalServerErr) {
        console.error(" [CRITICAL] HTTP Router Crash Protected:", globalServerErr);
        try { res.writeHead(500).end("Internal Server Error"); } catch(e){}
    }
