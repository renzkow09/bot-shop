...
http.createServer(async (req, res) => {
            ...
            try {
                ...
                if (data.action === 'edit_stat') {
                    ...
                    if (memoryStats.products && memoryStats.products[data.id]) {
                        ...
                        memoryStats.products[data.id] = { name: data.name, price: data.price, link: data.link, category: data.category || oldCat, stock: data.stock || "∞", desc: data.desc, upsellId: data.upsellId, upsellDiscount: data.upsellDiscount };
                        syncCloud();
                    }
                }
                    ...
                    const newCat = data.newCategory;
                    if (!oldCat || !newCat) throw new Error("Invalid parameters");
                    if (memoryStats.products) {
                        for (const [id, p] of Object.entries(memoryStats.products)) {
                    ...
                    if (!catToDelete) throw new Error("Invalid parameter");
                    if (memoryStats.products) {
                        for (const [id, p] of Object.entries(memoryStats.products)) {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        ",
            "        /* 📂 CATEGORIES SIDEBAR DESIGN */",
            "        .category-sidebar { width: 240px; background: #0c101d; border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; padding: 20px; flex-shrink: 0; transition: all 0.3s cubic-bezier(0.25, 1, 0.5, 1); display: flex; flex-direction: column; gap: 15px; transform: translate3d(0, 0, 0); backface-visibility: hidden; -webkit-font-smoothing: antialiased; }",
            "        .category-sidebar.collapsed { width: 0px !important; padding: 0px !important; border: none !important; margin: 0px !important; opacity: 0 !important; pointer-events: none !important; }",
            "        .category-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 14px; background: rgba(255,255,255,0.02); border-radius: 8px; cursor: pointer; font-size: 0.85em; font-weight: 600; color: var(--text-muted); transition: all 0.2s; border-left: 3px solid transparent; }",
            "        .category-item:hover { background: rgba(255,255,255,0.05); color: var(--text-main); transform: translateX(3px); }",
            "        .category-item.active { background: rgba(56,189,248,0.1); border-left-color: var(--accent-blue); color: var(--accent-blue); transform: translateX(3px); }",
            "        .category-collapsed-btn { display: none; width: 40px; background: #0c101d; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; cursor: pointer; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.3s; transform: translate3d(0,0,0); backface-visibility: hidden; -webkit-font-smoothing: antialiased; }",
            "        .category-collapsed-btn:hover { border-color: var(--accent-blue); box-shadow: 0 0 15px rgba(0,240,255,0.15); }",
            "        ",
            "        /* FEED ACTIVITY UI */",
            ...
            "          .category-sidebar { position: static; width: 100% !important; max-height: 250px; }",
            "          .category-collapsed-btn { display: none !important; }",
            "        }",
            "    </style>",
            "</head>",
            "<body>",
            ...
            "                       <input type='text' id='newProdCategory' list='existing-categories-list' placeholder='Category (e.g. ✨ PHOTOS)' style='flex:1; min-width:200px;'>",
            "                       <datalist id='existing-categories-list'></datalist>",
            "                   </div>",
            ...
            "                               <span style='font-size:0.85em; font-weight:800; text-transform:uppercase; letter-spacing:1px; color:var(--accent-blue);'>📂 Categories</span>",
            "                               <button class='btn-icon' style='padding:4px 8px; font-size:0.8em; margin:0; border-radius:6px;' onclick='window.toggleCategorySidebar()' title='Collapse Sidebar'>◀</button>",
            "                           </div>",
            "                           <div id='category-list' style='display:flex; flex-direction:column; gap:8px; overflow-y:auto; flex-grow:1; max-height:400px;'></div>",
            "                           <div style='border-top:1px solid rgba(255,255,255,0.05); padding-top:15px; display:flex; flex-direction:column; gap:8px;'>",
            "                               <button class='admin-btn' style='margin:0; font-size:0.85em; width:100%;' onclick='window.addNewCategory()'>➕ New Cat</button>",
            "                               <button class='admin-btn' style='margin:0; font-size:0.85em; width:100%;' onclick='window.renameCategoryDialog()'>✏️ Rename Cat</button>",
            "                               <button class='admin-btn' style='margin:0; font-size:0.85em; width:100%; background:rgba(239, 68, 68, 0.1); border-color:var(--accent-red); color:var(--accent-red);' onclick='window.deleteCategoryDialog()'>🗑️ Purge Cat</button>",
            "                           </div>",
            "                       </aside>",
            "                       <div class='category-collapsed-btn' id='category-sidebar-collapsed-btn' onclick='window.toggleCategorySidebar()'>",
            "                           <span style='writing-mode: vertical-lr; text-orientation: mixed; font-weight:800; font-size:0.75em; letter-spacing:2px; color:var(--accent-blue); transform: rotate(180deg);'>📁 CATEGORIES</span>",
            "                       </div>",
            "                       <div style='flex:1; display:flex; flex-direction:column; gap:15px;'>",
            "                           <div id='active-category-indicator' style='font-weight:800; font-size:1em; color:var(--accent-pink); letter-spacing:0.5px;'>Showing: All Products</div>",
            "                           <div class='product-grid' id='target-products' style='margin-top:0;'></div>",
            "                       </div>",
            "                   </div>",
            "               </div>",
            "",
            ...
            "            const collapsedBtn = document.getElementById('category-sidebar-collapsed-btn');",
            ...
            "            document.getElementById('active-category-indicator').innerText = 'Showing: ' + (cat === 'all' ? 'All Products' : cat);",
            "            buildStaticTables();",
            "        };",
            "        ",
            ...
            "          }",
            "",
            "          let catListHtml = `<div class=\"category-item ${selectedCategoryFilter === 'all' ? 'active' : ''}\" onclick=\"window.setCategoryFilter('all')\">",
            "              <span>🌍 All Assets</span>",
            "              <span class=\"nav-badge\" style=\"background:var(--accent-blue); box-shadow:0 0 10px var(--accent-blue);\">${totalActiveProducts}</span>",
            "          </div>`;",
            "",
            "          Array.from(uniqueCategories).sort().forEach(cat => {",
            "              const count = categoryCounts[cat] || 0;",
            "              catListHtml += `<div class=\"category-item ${selectedCategoryFilter === cat ? 'active' : ''}\" onclick=\"window.setCategoryFilter('${cat.replace(/'/g, \"\\\\'\")}')\">",
            "                  <span>${escapeHTML(cat)}</span>",
            "                  <span class=\"nav-badge\" style=\"background:var(--accent-pink); box-shadow:0 0 10px var(--accent-pink);\">${count}</span>",
            "              </div>`;",
            "          });",
            "          document.getElementById('category-list').innerHTML = catListHtml;",
            "",
            "          let dlHtml = '';",
            "          Array.from(uniqueCategories).forEach(cat => {",
            "              dlHtml += `<option value=\"${escapeHTML(cat)}\">`;",
            "          });",
            "          document.getElementById('existing-categories-list').innerHTML = dlHtml;",
            "",
            "          let prodHtml=''; ",
            "          if(rawStats.products){ ",
            "              Object.entries(rawStats.products).forEach(([id,p])=>{ ",
            "                  let pCat = p.category || '✨ ITEMS';",
            "                  if(selectedCategoryFilter !== 'all' && pCat !== selectedCategoryFilter) return;",
            "                  ",
