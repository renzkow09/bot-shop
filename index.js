    ...
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId === 'open_shop_channel') {
                ...
                if (!sanitizedName) sanitizedName = interaction.user.id;
        ...
        if (message.channel?.name?.startsWith('shop-')) {
            ...
            if (promoApplied || TEST_VOUCHERS[input] || input.length >= 8) {
                try {
                    ...
                    if (!promoApplied && !TEST_VOUVHERS[input]) {
...
http.createServer(async (req, res) => {
    ...
    // Anti-DoS / OOM (Cap de taille maximal sur flux entrants textuels)
