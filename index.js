...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "        body { font-family: 'Inter', sans-serif; background-color: var(--bg-main); background-image: radial-gradient(circle at 10% 50%, rgba(56, 189, 248, 0.03), transparent 30%), radial-gradient(circle at 90% 20%, rgba(168, 85, 247, 0.03), transparent 30%); color: var(--text-main); margin: 0; padding: 20px; min-height: 100vh; overflow-x: hidden; scroll-behavior: smooth; }",
            ...
            "        .container { max-width: 1400px; margin: 0 auto; animation: fadeInSmooth 0.8s cubic-bezier(0.16, 1, 0.3, 1); }",
            "        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.05); animation: slideIn 0.6s ease-out; }",
            "        .header h1 { font-size: 2.2em; font-weight: 800; margin: 0; letter-spacing: -1px; background: linear-gradient(135deg, #fff 0%, #00f0ff 50%, #a855f7 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }",
            ...
            "        .nav-menu { display: flex; gap: 8px; margin-bottom: 30px; background: rgba(15,23,42,0.4); padding: 8px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.05); overflow-x: auto; scrollbar-width: none; backdrop-filter: blur(24px); position: sticky; top: 20px; z-index: 100; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }",
            "        .nav-menu::-webkit-scrollbar { display: none; }",
            "        .nav-btn { background: transparent; border: none; color: var(--text-muted); font-size: 0.9em; font-weight: 600; padding: 12px 24px; border-radius: 12px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; letter-spacing: 0.5px; text-transform: uppercase; }",
            "        .nav-btn:hover { color: #fff; background: rgba(255,255,255,0.05); }",
            "        .nav-btn.active { color: #fff; background: linear-gradient(135deg, rgba(0,240,255,0.1) 0%, rgba(139,92,246,0.1) 100%); border: 1px solid rgba(0,240,255,0.3); box-shadow: inset 0 0 20px rgba(0,240,255,0.05), 0 0 15px rgba(0,240,255,0.2); }",
            ...
            "        .splash-screen { position: fixed; inset: 0; background: #030712; z-index: 9999; display: flex; flex-direction: column; justify-content: center; align-items: center; transition: opacity 1.2s cubic-bezier(0.8, 0, 0.2, 1), transform 1.2s cubic-bezier(0.8, 0, 0.2, 1); overflow: hidden; perspective: 1000px; }",
            "        .splash-glow { position: absolute; width: 80vw; height: 80vw; background: radial-gradient(circle, rgba(0, 240, 255, 0.1) 0%, transparent 60%); top: 50%; left: 50%; transform: translate(-50%, -50%); animation: pulseSplash 4s ease-in-out infinite alternate; pointer-events: none; }",
            "        @keyframes pulseSplash { 0% { transform: translate(-50%, -50%) scale(0.8); opacity: 0.5; } 100% { transform: translate(-50%, -50%) scale(1.2); opacity: 1; } }",
            "        .premium-logo-container { position: relative; z-index: 2; display: flex; flex-direction: column; align-items: center; }",
            "        .premium-logo { font-size: 4.5rem; font-weight: 800; letter-spacing: -2px; background: linear-gradient(135deg, #fff 0%, #00f0ff 50%, #8b5cf6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; opacity: 0; transform: translateY(30px) scale(0.95); filter: blur(15px); transition: all 1.5s cubic-bezier(0.16, 1, 0.3, 1) 0.2s; }",
            "        .premium-logo.show { opacity: 1; transform: translateY(0) scale(1); filter: blur(0); letter-spacing: 5px; }",
            "        .premium-subtitle { font-size: 1rem; color: #94a3b8; font-weight: 600; letter-spacing: 15px; text-transform: uppercase; margin-top: -5px; opacity: 0; transform: translateY(15px); filter: blur(5px); transition: all 1.5s cubic-bezier(0.16, 1, 0.3, 1) 0.8s; }",
            "        .premium-subtitle.show { opacity: 1; transform: translateY(0); filter: blur(0); }",
            "        .loader-wrapper { position: relative; width: 260px; height: 2px; background: rgba(255,255,255,0.05); border-radius: 2px; margin-top: 60px; overflow: hidden; opacity: 0; transition: opacity 1s ease 1.2s; }",
            "        .loader-wrapper.show { opacity: 1; }",
            "        .loader-bar { position: absolute; top: 0; left: 0; height: 100%; width: 0%; background: linear-gradient(90deg, transparent, #00f0ff, #fff); box-shadow: 0 0 20px #00f0ff; transition: width 0.1s cubic-bezier(0.4, 0, 0.2, 1); }",
            "        .loader-percentage { position: absolute; right: 0; top: 20px; color: #64748b; font-family: 'Inter', monospace; font-size: 0.8rem; font-weight: 600; letter-spacing: 2px; transition: opacity 0.5s ease; }",
            "        ",
            "        /* CHAT UI REFINED */",
            ...
            "            <h2 style=\"margin-top:0; color:var(--accent-purple);\">📦 Catalog Updated</h2>",
            "            <p class='text-muted' style='margin-bottom:25px;'>Deploy these changes to your Discord shop immediately?</p>",
            "            <button class='admin-btn' style='width:100%; margin-bottom:12px; background: linear-gradient(135deg, #a855f7, #d946ef); border:none;' onclick='window.triggerShopRefresh(); document.getElementById(\"syncModal\").style.display=\"none\";'>🚀 Sync with Discord</button>",
            "            <button class='admin-btn' style='width:100%; background:transparent; border-color:rgba(255,255,255,0.1); color:var(--text-muted);' onclick='document.getElementById(\"syncModal\").style.display=\"none\";'>Save for later</button>",
            "        </div>",
            "    </div>",
            "",
            ...
            "           <div class='box'><h2>🕒 Peak Execution Hours</h2><div style='height:280px; margin-top:20px;'><canvas id='hourlyChart'></canvas></div></div>",
            "           <div style='display:grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap:25px;'>",
            "               <div class='box'><h2>🏆 Top Performing Assets</h2><div style='height:300px; margin-top:20px;'><canvas id='topProductsBarChart'></canvas></div></div>",
            "               <div class='box'><h2>🏷️ Sector Revenue</h2><div style='height:300px; margin-top:20px;'><canvas id='categoryRevenueChart'></canvas></div></div>",
            "           </div>",
            "       </div>",
            "",
            ...
            "           </div>",
            "           <div class='box'><h2>📦 Active Asset Matrix</h2><div class='product-grid' id='target-products' style='margin-top:20px;'></div></div>",
            "       </div>",
            "            ",
            ...
            "           </div>",
            "            ",
            "           <div class='box'><h2>⚡ Architecture Reset</h2><p class='text-muted'>Purge existing Discord UI and redeploy the latest aesthetic parameters.</p><button class='admin-btn' style='background:linear-gradient(135deg, rgba(168,85,247,0.2), rgba(217,70,239,0.2)); color:#fff; border-color:var(--accent-purple); width:100%; padding:20px; font-size:1.1em;' onclick='window.triggerShopRefresh()'>🚀 Execute Deployment</button></div>",
            "           ",
            "           <div class='box'>",
            "               <h2>🎟️ Custom Vouchers</h2>",
            ...
            "           ",
            "           setTimeout(() => {",
            "               const logo = document.getElementById('premium-logo');",
            "               const sub = document.getElementById('premium-subtitle');",
            ...
            "                   if(loadTxt) loadTxt.innerText = Math.floor(progress).toString().padStart(2, '0') + '%';",
            "               }",
            "           }, 40);",
            "           ",
            "           try{",
            ...
            "                           if(splash) {",
            "                               splash.style.transform = 'scale(1.1) translateY(-20px)';",
            ...
            "        }",
            "            ",
            "        window.editTodayEarnings = async function() { const current = document.getElementById('ui-today-rev').innerText.replace('€', ''); const n = prompt(\"Override Daily Earning Parameter (€):\", current); if(n !== null) { const parsed = parseFloat(n); if(!isNaN(parsed)) { await window.executeAction({action:'edit_today_earnings', value: parsed}); } } };",
            "",
            "        window.approveReview = async function(id) { await window.executeAction({action:'approve_review', id:id}); };",
            "        window.rejectReview = async function(id) { const reason = prompt('Reject Review. Specify reason for user log:'); if(reason !== null) await window.executeAction({action:'reject_review', id:id, reason:reason}); };",
            "",
            ...
            "        window.editReferralCount = async function(id, current) { const n = prompt('Overwrite referral node integer:', current); if(n !== null) { const parsed = parseInt(n); if(!isNaN(parsed)) { await window.executeAction({action:'edit_referral_count', userId:id, newCount: parsed}); } } };",
            "        ",
            ...
            "            if(!n||!p) return alert('Designation & Value required'); ",
            ...
            "        };",
            "        ",
            "        window.deleteProduct = async function(id) { if(confirm('Purge asset from network?')) await window.executeAction({action:'delete_product', id:id}, true); };",
            "        ",
            ...
            "        ",
            "        window.sendReview = async function() { const author = document.getElementById('rev-author').value; const rating = document.getElementById('rev-rating').value; const text = document.getElementById('rev-msg').value; if(!author || !text) return alert('Parameters missing'); await window.executeAction({ action: 'post_review', author: author, rating: rating, text: text }); document.getElementById('rev-author').value = ''; document.getElementById('rev-msg').value = ''; };",
            ...
            "        }",
            "        window.modAction = async function(action, userId, extra) { extra = extra || {}; let payload = { action: action, userId: userId, pin: PIN }; if (extra.channelId) payload.channelId = extra.channelId; if (extra.duration) payload.duration = extra.duration; if (action === 'warn') { payload.reason = prompt(\"Input warning parameter (User will be DM'd)\"); if (!payload.reason) return; } else if (action === 'clear_warns') { if (!confirm('Purge all risk logs for this node?')) return; } else if (action === 'mute') { if(!payload.duration) payload.duration = prompt('Timeout duration (minutes)?', '60'); payload.reason = prompt('Reason for timeout?'); if (!payload.duration || !payload.reason) return; } else if (action === 'kick' || action === 'ban') { payload.reason = prompt('Reason for ' + action + '?'); if (!payload.reason || !confirm('Execute ' + action + '?')) return; } else if (action === 'toggle_blacklist') { if (!confirm('Toggle shop access for this node?')) return; } else if (action === 'close_channel') { if (!confirm('Sever this link?')) return; } try { const res = await fetch('/api/action', { method: 'POST', body: JSON.stringify(payload) }); if (res.ok) { showToast('Action Successful'); setTimeout(function() { window.loadAllMembers(); }, 1000); } else showToast('Action Failed', 'error'); } catch(e) { showToast('Network Error', 'error'); } };",
            "        window.refundTx = async function(date, username) { if(confirm('Reverse this transaction? Yield will be adjusted.')) { await window.executeAction({action: 'refund_tx', date: date, username: username}); } };",
            "        window.runDiagnostics = async function() { document.getElementById('ui-upstash-status').innerText = '⏳ Executing...'; document.getElementById('ui-upstash-status').className = 'value text-muted'; document.getElementById('ui-rewarble-status').innerText = '⏳ Executing...'; document.getElementById('ui-rewarble-status').className = 'value text-muted'; document.getElementById('ui-discord-ws').innerText = '-- ms'; try { const res = await fetch('/api/monitoring'); const data = await res.json(); const upstashCard = document.getElementById('card-upstash'); const rewarbleCard = document.getElementById('card-rewarble'); if (data.upstash.status === 'online') { document.getElementById('ui-upstash-status').innerHTML = '🟢 Optimal'; document.getElementById('ui-upstash-status').className = 'value text-green'; upstashCard.style.borderLeft = '4px solid var(--accent-green)'; } else { document.getElementById('ui-upstash-status').innerHTML = '🔴 Down'; document.getElementById('ui-upstash-status').className = 'value text-red'; upstashCard.style.borderLeft = '4px solid var(--accent-red)'; } document.getElementById('ui-upstash-ping').innerText = 'Latency: ' + data.upstash.latency + ' ms'; if (data.rewarble.status === 'online') { document.getElementById('ui-rewarble-status').innerHTML = '🟢 Optimal'; document.getElementById('ui-rewarble-status').className = 'value text-green'; rewarbleCard.style.borderLeft = '4px solid var(--accent-green)'; } else { document.getElementById('ui-rewarble-status').innerHTML = '🔴 Down'; document.getElementById('ui-rewarble-status').className = 'value text-red'; rewarbleCard.style.borderLeft = '4px solid var(--accent-red)'; } document.getElementById('ui-rewarble-ping').innerText = 'Latency: ' + data.rewarble.latency + ' ms'; document.getElementById('ui-discord-ws').innerText = data.discord.ws_ping + ' ms'; } catch(e) { showToast('Diagnostics Failed', 'error'); } };",
            ...
            "        window.deletePromo = async function(code) { if(confirm('Purge voucher ' + decodeURIComponent(code) + '?')) { await window.executeAction({ action: 'delete_promo', name: decodeURIComponent(code) }); } };",
            ...
            "        window.openDirectContact = async function(id) { const msg = prompt('Input DM payload:'); if(msg) await window.executeAction({action:'send_dm', userId: id, message: msg}); };",
            ...
            "        window.manageVip = async function(userId, action) { if(action === 'add') { await window.executeAction({action: 'add_vip_days', userId: userId, days: 7}); } else if(action === 'revoke') { if(confirm('Revoke VIP status for this node?')) { await window.executeAction({action: 'revoke_vip', userId: userId}); } } };",
            "",
