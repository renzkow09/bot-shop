...
http.createServer(async (req, res) => {
    ...
    if (req.url === '/api/action' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            try {
                ...
                // MODIFICATION DE PRODUIT INTÉGRÉE
                if (data.action === 'edit_product') {
                    if (memoryStats.products && memoryStats.products[data.id]) {
                        memoryStats.products[data.id] = { name: data.name, price: data.price, link: data.link };
                        syncCloud();
                    }
                }
    ...
    if (req.url === '/dashboard' || req.url === '/') {
        ...
        const dashboardHTML = [
            ...
            "* { box-sizing: border-box; } body { font-family: 'Inter', sans-serif; background-color: var(--bg-main); color: var(--text-main); margin: 0; padding: 20px; min-height: 100vh; overflow-x: hidden; }",
            ...
            ".btn-icon { background: var(--bg-card); border: 1px solid var(--border-color); color: white; padding: 8px 15px; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; }",
            ".btn-icon:hover { background: rgba(255,255,255,0.15); transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.3); }",
            ".nav-menu { display: flex; gap: 10px; margin-bottom: 30px; background: var(--bg-card); padding: 10px; border-radius: 12px; border: 1px solid var(--border-color); overflow-x: auto; scrollbar-width: none; animation: slideIn 0.6s ease-out; }",
            ...
            ".nav-btn:hover { color: #fff; background: rgba(255,255,255,0.05); transform: scale(1.05); }",
            ...
            ".card { background: var(--bg-card); padding: 20px; border-radius: 12px; border: 1px solid var(--border-color); transition: all 0.3s ease; position: relative; overflow: hidden; }",
            ".card:hover { transform: translateY(-5px); box-shadow: 0 8px 25px rgba(0,0,0,0.4); border-color: rgba(255,255,255,0.2); }",
            ...
            ".box { background: var(--bg-card); padding: 20px; border-radius: 12px; border: 1px solid var(--border-color); margin-bottom:20px; transition: all 0.3s ease; }",
            ".box:hover { box-shadow: 0 5px 20px rgba(0,0,0,0.3); }",
            "table { width: 100%; border-collapse: collapse; } th, td { padding: 12px; text-align: left; border-bottom: 1px solid rgba(255,255,255,0.05); transition: background 0.2s; } tr:hover td { background: rgba(255,255,255,0.02); }",
            "input, textarea, select { width: 100%; background: rgba(0,0,0,0.3); border: 1px solid var(--border-color); color: white; padding: 10px; border-radius: 6px; transition: all 0.3s ease; }",
            "input:focus, textarea:focus, select:focus { border-color: var(--accent-blue); box-shadow: 0 0 10px rgba(56,189,248,0.2); outline: none; }",
            ".admin-btn { background: var(--accent-blue); color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-weight: bold; margin-top:10px; transition: all 0.3s ease; }",
            ".admin-btn:hover { filter: brightness(1.2); transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.4); }",
            ".text-green { color: var(--accent-green); } .text-muted { color: var(--text-muted); }",
            "@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }",
            ".modal { display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:10000; justify-content:center; align-items:center; animation: fadeInSmooth 0.3s ease-out; }",
            ".modal-content { background:var(--bg-main); padding:30px; border-radius:12px; border:1px solid var(--accent-blue); text-align:center; max-width:400px; box-shadow: 0 10px 40px rgba(0,0,0,0.8); transform: scale(0.95); animation: zoomIn 0.3s forwards; }",
            ...
            "<div class='modal' id='syncModal'><div class='modal-content'><h2>📦 Catalog Saved!</h2><p class='text-muted'>Apply these changes to your Discord shop channel right now?</p><button class='admin-btn' style='background:var(--accent-purple); width:100%;' onclick='window.triggerShopRefresh(); document.getElementById(\"syncModal\").style.display=\"none\";'>🔄 Setup & Clear Old Menu</button><button class='admin-btn' style='background:transparent; border:1px solid var(--border-color); width:100%;' onclick='document.getElementById(\"syncModal\").style.display=\"none\";'>Skip</button></div></div>",
            "<div class='container' id='dashboard-container' style='display:none;'><div class='header'><h1>Nexus Dashboard</h1><div class='controls'><button class='btn-icon' onclick='window.toggleStealth()' id='stealthBtn'>👁️ Stealth</button><div class='live-status btn-icon' style='background:var(--accent-blue); border:none;'><span id='live-tickets-count'>0</span> Live Tickets</div></div></div>",
            "<div class='nav-menu'><button class='nav-btn active' onclick='window.switchTab(\"overview\", this)'>📊 Overview</button><button class='nav-btn' onclick='window.switchTab(\"transactions\", this)'>💳 Transactions</button><button class='nav-btn' onclick='window.switchTab(\"products\", this)'>📦 Products</button><button class='nav-btn' onclick='window.switchTab(\"audience\", this)'>👥 Audience</button><button class='nav-btn' onclick='window.switchTab(\"referrals\", this)'>🔗 Referrals</button><button class='nav-btn' onclick='window.switchTab(\"moderation\", this)'>🛡️ Moderation</button><button class='nav-btn' onclick='window.switchTab(\"admin\", this)'>⚙️ Admin Config</button></div>",
            "<div id='overview' class='tab-content active'><div class='stats-grid'><div class='card green'><h3>Today's Earnings</h3><div class='value money text-green' id='ui-today-rev'>€0</div></div><div class='card blue'><h3>Total Earnings</h3><div class='value money text-blue' id='ui-total-rev'>€0</div></div><div class='card pink'><h3>Conversion Rate</h3><div class='value text-pink' id='ui-conv-rate'>0%</div></div><div class='card orange'><h3>Online / Total</h3><div class='value text-orange' id='ui-online-total'>0</div></div><div class='card purple'><h3>Retention Rate</h3><div class='value text-purple' id='ui-retention'>0%</div></div></div><div class='stats-grid'><div class='card purple'><h3>Tickets Opened</h3><div class='value' id='ui-tickets-opened'>0</div></div><div class='card red'><h3>Drop-off Rate</h3><div class='value text-red' id='ui-dropoff'>0%</div></div><div class='card orange'><h3>Peak Sales Hour</h3><div class='value' id='ui-peak-hour'>N/A</div></div></div><div class='box'><h2>📈 Revenue Timeline</h2><canvas id='salesChart' style='height:250px;'></canvas></div></div>",
            "<div id='transactions' class='tab-content'><div class='box'><h2>🛒 Recent Transactions</h2><table><thead><tr><th>Customer</th><th>Product</th><th>Price</th><th>Date</th></tr></thead><tbody id='target-tx'></tbody></table></div></div>",
            "<div id='products' class='tab-content'><div class='box'><h2>➕ Add / Edit Product</h2><div style='display:flex; gap:10px; flex-wrap:wrap; margin-bottom:15px;'><input type='hidden' id='editProdId'><input type='text' id='newProdName' placeholder='Product Name' style='flex:1;'><input type='text' id='newProdPrice' placeholder='Price in €' style='width:120px;'><input type='text' id='newProdLink' placeholder='Delivery Link' style='flex:2;'><button class='admin-btn' style='margin:0;' onclick='window.saveProduct()' id='saveProdBtn'>➕ Add</button><button class='admin-btn' style='margin:0; background:var(--accent-red); display:none;' onclick='window.cancelEdit()' id='cancelEditBtn'>Cancel</button></div></div><div class='box'><h2>📦 Current Catalog</h2><table><thead><tr><th>ID</th><th>Name</th><th>Price</th><th>Link</th><th>Actions</th></tr></thead><tbody id='target-products'></tbody></table></div></div>",
            "<div id='audience' class='tab-content'><div class='box'><h2>📥 Latest Joins</h2><table><thead><tr><th>Username</th><th>Date</th></tr></thead><tbody id='target-joins'></tbody></table></div></div>",
            "<div id='referrals' class='tab-content'><div class='box'><h2>🔗 Referral Threshold</h2><input type='number' id='ref-threshold' style='width:100px;'><button class='admin-btn' onclick='window.updateRefThreshold()'>💾 Save</button></div><div class='box'><h2>🏆 Top Inviters</h2><table><thead><tr><th>User</th><th>Invites</th><th>Rewards</th><th>Invited Users</th></tr></thead><tbody id='target-referrals'></tbody></table></div></div>",
            "<div id='moderation' class='tab-content'><div class='box'><h2>🔎 Member Directory</h2><button class='admin-btn' onclick='window.loadAllMembers()'>🔄 Load Members</button><div id='memberResults'></div></div></div>",
            "<div id='admin' class='tab-content'><div class='box'><h2>⚡ 1-Click Shop Setup</h2><p class='text-muted'>Clear the old menu and instantly post the new aesthetic setup.</p><button class='admin-btn' style='background:var(--accent-purple); width:100%; padding:15px;' onclick='window.triggerShopRefresh()'>🔄 Setup and clear old menu</button></div><div class='box'><h2>🎟️ Promo Codes</h2><div style='display:flex; gap:10px;'><input type='text' id='promoName' placeholder='CODE'><input type='number' id='promoDiscount' placeholder='% Off'><input type='number' id='promoLimit' placeholder='Uses'><button class='admin-btn' style='margin:0;' onclick='window.createPromo()'>➕ Create</button></div><table><thead><tr><th>Code</th><th>Discount</th><th>Usage</th><th>Action</th></tr></thead><tbody id='target-promos'></tbody></table></div><div class='box'><h2>🌟 Post Customer Review</h2><input type='text' id='rev-author' placeholder='Author'><select id='rev-rating'><option value='5'>5/5 ⭐</option><option value='4'>4/5 ⭐</option></select><textarea id='rev-msg' placeholder='Review Text'></textarea><button class='admin-btn' onclick='window.sendReview()'>📤 Publish Review</button></div></div>",
            "</div><script>",
            "let PIN='', rawStats={}, PRODUCT_DATA={}, lastTxCount=0, currentMonthRevenue=0, userGoal=500, salesChart; ",
            "async function initDashboard(){ try{ const res=await fetch('/api/init-data'); if(!res.ok)throw new Error('Err'); const data=await res.json(); rawStats=data.memoryStats; PRODUCT_DATA=data.PRODUCT_DATA; currentMonthRevenue=data.monthRevenue; PIN=data.PIN; lastTxCount=rawStats.total_transactions||0; document.getElementById('ui-today-rev').innerText='€'+data.todayRevenue; document.getElementById('ui-total-rev').innerText='€'+(rawStats.total_revenue||0); document.getElementById('ui-conv-rate').innerText=data.conversionRate+'%'; document.getElementById('ui-online-total').innerHTML=data.onlineCount+\" <span style='font-size:0.5em;color:var(--text-muted);'>/ \"+data.memberCount+\"</span>\"; document.getElementById('ui-retention').innerText=data.retentionRate+'%'; document.getElementById('ui-tickets-opened').innerText=data.ticketsOpened; document.getElementById('ui-dropoff').innerText=data.dropOffRate+'%'; document.getElementById('ui-peak-hour').innerText=data.peakHourStr; buildStaticTables(); setTimeout(()=>{ document.getElementById('loading-screen').style.opacity='0'; setTimeout(()=>{ document.getElementById('loading-screen').style.display='none'; document.getElementById('dashboard-container').style.display='block'; }, 500); }, 500); }catch(e){ alert('API Error'); } }",
            "function escapeHTML(str){ return str ? String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') : ''; }",
            "function buildStaticTables(){ let txHtml=''; if(rawStats.recent_transactions&&rawStats.recent_transactions.length>0){ rawStats.recent_transactions.forEach(tx=>{ txHtml+='<tr><td>'+escapeHTML(tx.username)+'</td><td>'+escapeHTML(tx.product)+'</td><td class=\"text-green font-bold\">€'+tx.price+'</td><td>'+tx.date+'</td></tr>'; }); } document.getElementById('target-tx').innerHTML=txHtml; let prodHtml=''; if(rawStats.products){ Object.entries(rawStats.products).forEach(([id,p])=>{ prodHtml+='<tr><td>'+id+'</td><td><strong>'+escapeHTML(p.name)+'</strong></td><td class=\"text-green\">'+(p.price==='Custom'?'Custom':'€'+p.price)+'</td><td class=\"text-muted\">'+(p.link?p.link.substring(0,30)+'...':'None')+'</td><td><button class=\"admin-btn\" style=\"padding:5px 10px; margin:0 5px 0 0;\" onclick=\"window.editProduct(\\''+id+'\\')\">✏️ Edit</button><button class=\"admin-btn\" style=\"background:var(--accent-red); padding:5px 10px; margin:0;\" onclick=\"window.deleteProduct(\\''+id+'\\')\">🗑️</button></td></tr>'; }); } document.getElementById('target-products').innerHTML=prodHtml; let jHtml=''; if(rawStats.recent_joins){ rawStats.recent_joins.forEach(u=>{ jHtml+='<tr><td>'+escapeHTML(u.username)+'</td><td>'+u.date+'</td></tr>'; }); } document.getElementById('target-joins').innerHTML=jHtml; let promHtml=''; if(rawStats.promo_codes){ for(const code in rawStats.promo_codes){ const info=rawStats.promo_codes[code]; promHtml+='<tr><td><strong>'+escapeHTML(code)+'</strong></td><td class=\"text-green\">-'+info.discount+'%</td><td>'+info.used+'/'+info.limit+'</td><td><button onclick=\"window.deletePromo(\\\''+encodeURIComponent(code)+'\\\')\">🗑️</button></td></tr>'; } } document.getElementById('target-promos').innerHTML=promHtml; document.getElementById('ref-threshold').value=rawStats.settings?.invite_reward_threshold||10; let refHtml=''; if(rawStats.referrals){ Object.entries(rawStats.referrals).forEach(([id,r])=>{ let list=r.invited.slice(0,3).map(u=>escapeHTML(u.username)).join(', '); refHtml+='<tr><td>'+escapeHTML(r.username||id)+'</td><td class=\"text-green font-bold\">'+r.count+'</td><td>'+r.total_rewards+'</td><td class=\"text-muted\">'+list+'</td></tr>'; }); } document.getElementById('target-referrals').innerHTML=refHtml; }",
            ...
            "window.switchTab = function(tabId, btn) { document.querySelectorAll('.tab-content').forEach(el=>el.classList.remove('active')); document.querySelectorAll('.nav-btn').forEach(el=>el.classList.remove('active')); document.getElementById(tabId).classList.add('active'); btn.classList.add('active'); };",
            "function showToast(msg) { const t=document.getElementById('toast'); t.innerText=msg; t.style.bottom='20px'; setTimeout(()=>{t.style.bottom='-100px';}, 3000); }",
            "window.executeAction = async function(p, showModal=false) { p.pin=PIN; const res=await fetch('/api/action',{method:'POST',body:JSON.stringify(p)}); if(res.ok) { if(showModal){ document.getElementById('syncModal').style.display='flex'; }else{ showToast('✅ Success!'); setTimeout(()=>location.reload(),1000); } } else { showToast('❌ Error'); } };",
